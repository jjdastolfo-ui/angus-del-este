...
"mediciones" — una fila por medición. Un mismo animal puede tener varias.
  rp     (obligatorio)
  tipo   (obligatorio) uno de estos, según lo que mida la columna:
           CE        circunferencia escrotal, en cm (sólo machos)
           ALTURA    altura de anca, en cm
           CC        condición corporal, escala 1 a 9
           DOCILIDAD escala 1 a 6
           AOB       área de ojo de bife por ecografía, en cm²
           GD        grasa dorsal por ecografía, en mm (también "espesor de grasa")
           GC        grasa de cadera por ecografía, en mm (también "grasa anca" o "P8")
           GI        grasa intramuscular o marmoleo, en % (también "%GI", "IMF", "marbling")
  valor  (número, obligatorio)
  fecha  (AAAA-MM-DD)

  UNA ECOGRAFÍA DE CALIDAD DE CARNE trae varias de estas columnas juntas
  (%GI, AOB, GD, GC). Cada una es una medición distinta: generá UNA FILA POR
  COLUMNA Y POR ANIMAL, no las mezcles ni elijas una sola.
...

{
  ...
  "columnas_medicion": { "columna del archivo": "TIPO" },
...
- NUNCA fuerces una columna a un campo que no le corresponde. Si una medición no
  entra en ninguno de los tipos de arriba, dejala afuera y decilo en
  "observacion". Cargar grasa de cadera como circunferencia escrotal, por
  ejemplo, arruina los datos del animal.
- Si el archivo trae VARIAS mediciones por animal (una ecografía típica),
  devolvé el mapeo de UNA de ellas y listá las demás en "columnas_medicion",
  con la forma {"NOMBRE DE COLUMNA": "TIPO"} — el sistema genera una fila por
  cada una.`;
...
function normalizar(datos, plan) {
  ...
  // Varias mediciones por animal (ecografía): una fila por columna.
  const extras = plan.columnas_medicion && Object.keys(plan.columnas_medicion).length
    ? plan.columnas_medicion : null;
  ...
  for (const cruda of datos.filas) {
    ...
    // Las demás columnas de medición generan sus propias filas, con el mismo
    // animal y la misma fecha.
    if (extras && fila.rp) {
      for (const [columna, tipo] of Object.entries(extras)) {
        let v = cruda[columna];
        if (v === undefined) {
          const k = Object.keys(cruda).find(x => x.toLowerCase().trim() === String(columna).toLowerCase().trim());
          if (k) v = cruda[k];
        }
        const n = aNumero(v);
        if (n === null) continue;
        filas.push({ rp: fila.rp, tipo: String(tipo).toUpperCase(), valor: n,
...
// Cada medición tiene su rango: así un dato puesto en el tipo equivocado no
// entra en silencio. Los límites son generosos a propósito.
const RANGO_MEDICION = {
  CE:        [15, 55],   // cm de perímetro escrotal
  ALTURA:    [70, 170],  // cm
  CC:        [1, 9],     // escala
  DOCILIDAD: [1, 6],     // escala
  AOB:       [20, 130],  // cm² de ojo de bife
  GD:        [0.5, 30],  // mm de grasa dorsal
  GC:        [0.5, 40],  // mm de grasa de cadera
  GI:        [0.5, 15]   // % de grasa intramuscular
};
...
function validar(tipo, filas) {
  ...
  filas.forEach((f, i) => {
    ...
    // Rango propio del tipo de medición
    if (tipo === "mediciones" && f.tipo && RANGO_MEDICION[f.tipo]) {
      const [min, max] = RANGO_MEDICION[f.tipo];
      if (f.valor < min || f.valor > max) {
        fuera = `${f.tipo} = ${f.valor} está fuera de lo esperable (${min} a ${max})`;
      }
    }
...
function resumir(tipo, buenas, problemas, plan, existe) {
  const l = [];
  l.push(`${plan.foto ? "📷" : "📄"} ${plan.observacion || "Archivo leído"}`);
  l.push("");
  l.push(`Encontré *${buenas.length} ${ETIQUETA[tipo] || "registros"}* para cargar.`);

  if (tipo === "mediciones" && buenas.length) {
    const porTipo = {};
    buenas.forEach(b => { porTipo[b.tipo] = (porTipo[b.tipo] || 0) + 1; });
    const NOM = { CE:"circunferencia escrotal", ALTURA:"altura", CC:"condición corporal",
                  DOCILIDAD:"docilidad", AOB:"área de ojo de bife", GD:"grasa dorsal",
                  GC:"grasa de cadera", GI:"grasa intramuscular" };
    l.push(`De ${new Set(buenas.map(b => b.rp)).size} animales:`);
    Object.entries(porTipo).forEach(([t, n]) => {
      const vals = buenas.filter(b => b.tipo === t).map(b => b.valor);
      const prom = vals.reduce((a, b) => a + b, 0) / vals.length;
      l.push(`   ${NOM[t] || t}: ${n} · promedio ${prom.toFixed(1)}`);
    });
...
// ── IMÁGENES ─────────────────────────────────────────────────────────────────
// La pizarra de la manga, un remito escrito a mano, la pantalla de la balanza.
// Acá sí la IA devuelve las filas directamente: una foto trae diez o veinte
// registros, no mil, y no hay columnas que mapear — hay que leer lo que se ve.

const IMAGENES_OK = ["image/jpeg", "image/png", "image/gif", "image/webp"];

async function analizarImagen({ base64, tipo, instruccion, anthropic, rpsExistentes }) {
  const limpio = String(tipo || "").split(";")[0].trim().toLowerCase();
  if (!IMAGENES_OK.includes(limpio)) {
    return { ok: false, mensaje: `No puedo leer imágenes ${limpio || "de ese formato"}. Sacá la foto directo con la cámara de WhatsApp y mandala de nuevo.` };
  }

  const prompt = `Sos un asistente de un sistema de gestión ganadera. Te paso una foto que
sacó un productor en el campo. Puede ser la pizarra de la manga, una planilla escrita a
mano, la pantalla de una balanza, un remito o un certificado.

${instruccion ? `LO QUE DIJO EL USUARIO: "${instruccion}"` : "El usuario no aclaró nada."}

${ESQUEMA}

Leé la foto y respondé SOLO con un objeto JSON, sin texto alrededor y sin backticks:

{
  "tipo": "pesadas" | "animales" | "mediciones" | "sanidad" | "desconocido",
  "confianza": "alta" | "media" | "baja",
  "filas": [ { campos del esquema, uno por registro que veas } ],
  "observacion": "una línea sobre qué es la foto, en castellano rioplatense",
  "dudas": ["lecturas que no estás seguro, con el dato como lo leíste"]
}

REGLAS:
- Transcribí SOLO lo que ves. Si un número está borroso o tapado, no lo inventes:
  dejá ese registro afuera y anotalo en "dudas".
- Las fechas en formato AAAA-MM-DD. Si la foto no tiene fecha, no pongas el campo.
- Si el usuario dijo el contexto de la pesada (destete, año, 18 meses), usalo en
  todas las filas.
- Si la foto no tiene datos ganaderos legibles, poné tipo "desconocido".
- Es preferible traer 5 registros seguros que 20 con la mitad adivinada.`;
  ...
  const r = await anthropic.messages.create({
    ...
    messages: [{
      role: "user",
      content: [
        { type: "image", source: { type: "base64", media_type: limpio, data: base64 } },
  ...
  const txt = r.content.map(c => c.text || "").join("").trim();
  const sinCercos = txt.replace(/^```(?:json)?/i, "").replace(/```$/, "").trim();
  let plan;
  try { plan = JSON.parse(sinCercos); }
  catch (e) {
    const m = sinCercos.match(/\{[\s\S]*\}/);
    if (!m) return { ok: false, mensaje: "No pude leer la foto. Probá con más luz o más de cerca." };
    plan = JSON.parse(m[0]);
  }

  if (plan.tipo === "desconocido" || !Array.isArray(plan.filas) || !plan.filas.length) {
    return { ok: false, mensaje: `No encontré datos que pueda cargar.\n${plan.observacion || ""}\n\nSi es una planilla, probá enfocándola derecha y que se lean los números.` };
  }

  // Las filas ya vienen con nuestros nombres de campo, pero los valores hay que
  // normalizarlos igual: la IA puede devolver "198,5" o "15/03/2026".
  const filas = plan.filas.map(f => {
    const limpia = {};
    for (const [k, v] of Object.entries(f)) {
      if (v === null || v === undefined || v === "") continue;
      if (FECHAS.has(k)) limpia[k] = aFecha(v, null);
      else if (NUMERICOS.has(k)) limpia[k] = aNumero(v);
      else limpia[k] = String(v).trim();
    }
    return limpia;
  });
  ...
  if (rpsExistentes && buenas.length) {
    const set = new Set([...rpsExistentes].map(x => String(x).toUpperCase()));
    existe = { faltantes: [...new Set(buenas.map(b => String(b.rp).toUpperCase()).filter(rp => !set.has(rp)))] };
  }

  let mensaje = resumir(plan.tipo, buenas, problemas, plan, existe);
  if (plan.dudas && plan.dudas.length) {
    mensaje = mensaje.replace("\n\n¿Los cargo?",
      `\n\n🔍 No pude leer bien: ${plan.dudas.slice(0, 4).join(", ")}\n\n¿Los cargo?`);
  }

  return { ok: true, tipo: plan.tipo, plan, filas: buenas, problemas, mensaje };
}
...
module.exports = { analizar, analizarImagen, aFecha, aNumero, normalizar, validar };
