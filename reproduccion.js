...
function init(db) {
  ...
  // Cada vez que se agrega una columna al esquema de arriba, va acá también.
  [["repro_toros", "producto_stock", "TEXT"],
   ["repro_vacas", "toro_repaso_id", "INTEGER"],
...
function guardarToro(db, t) {
  ...
  // Con ON CONFLICT DO UPDATE, lastInsertRowid devuelve un rowid que no
  // corresponde al registro: hay que buscarlo siempre por su clave real.
  const fila = db.prepare("SELECT id FROM repro_toros WHERE rp=? AND nombre=?").get(t.rp || "", t.nombre);
  if (!fila) throw new Error(`No pude guardar el toro ${t.nombre}`);
  return fila.id;
}
...
function registrarParto(db, datos) {
  ...
  // Tampoco puede existir en otro campo de la misma empresa: cada campo tiene
  // su base, así que el server pasa la función que sabe consultar los hermanos.
  if (typeof datos.verificarRpUnico === "function") {
    const ch = datos.verificarRpUnico(datos.rp_cria);
    if (ch) return { ok: false, error: `El RP ${datos.rp_cria} ya existe en ${ch.nombre}.` };
  }
