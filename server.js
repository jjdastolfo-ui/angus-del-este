...
// Migration: add hbu column for INIA registry number
try { db.exec("ALTER TABLE animales ADD COLUMN hbu TEXT"); } catch(e) { /* already exists */ }
// Migrate HBU from notas field if present
try {
  const conHbuNotas = db.prepare("SELECT id, notas FROM animales WHERE notas LIKE '%HBU:%' AND (hbu IS NULL OR hbu = '')").all();
  for (const a of conHbuNotas) {
    const m = a.notas.match(/HBU:(\w+)/);
    if (m) db.prepare("UPDATE animales SET hbu = ? WHERE id = ?").run(m[1], a.id);
  }
...
app.put("/api/hbu/:id", (req, res) => {
  const { hbu, madre_hba, padre_hba } = req.body;
  const sets = [];
  const vals = [];
...
app.post("/api/informe-inia/18meses", async (req, res) => {
  ...
  try {
    ...
    for (const rp of rps) {
      const animal = buscarAnimal(rp);
      if (!animal) { noEncontrados.push(rp); continue; }
      const p18 = db.prepare("SELECT peso FROM pesadas WHERE animal_id = ? AND contexto = '18MESES' ORDER BY fecha DESC LIMIT 1").get(animal.id);
      const pano = db.prepare("SELECT peso FROM pesadas WHERE animal_id = ? AND contexto = 'AÑO' ORDER BY fecha DESC LIMIT 1").get(animal.id);
      const ce = db.prepare("SELECT valor FROM mediciones WHERE animal_id = ? AND tipo = 'CE' ORDER BY fecha DESC LIMIT 1").get(animal.id);
      const alt = db.prepare("SELECT valor FROM mediciones WHERE animal_id = ? AND tipo = 'ALTURA' ORDER BY fecha DESC LIMIT 1").get(animal.id);
      const frm = db.prepare("SELECT valor FROM mediciones WHERE animal_id = ? AND tipo = 'FRAME' ORDER BY fecha DESC LIMIT 1").get(animal.id);
      animal.peso_18m = p18 ? p18.peso : null;
      animal.peso_ano = pano ? pano.peso : null;
      animal.ce = ce ? ce.valor : null;
      animal.altura = alt ? alt.valor : null;
      animal.frame = frm ? frm.valor : null;
