...
/*
  POST /api/informe-inia/completar-18meses
  
  Recibe el Excel 299 de INIA formato 18 meses (pre-llenado) y lo completa con datos del sistema.
  Columnas del formato 18m INIA:
  - Col 9 (J): RP del animal
  - Col 10 (K): Peso 18 meses ← completar
  - Col 13 (N): CE ← completar
  - Col 16 (Q): Manejo ← completar
  - Col 18 (S): Lote ← completar
  - Col 25 (Z): Padre RP ← completar si falta
*/
app.post("/api/informe-inia/completar-18meses", express.raw({ type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', limit: '10mb' }), async (req, res) => {
  if (!ExcelJS) return res.status(500).json({ error: "exceljs no instalado" });
  ...
  try {
    ...
    // Detectar fila de inicio de datos: buscar primera fila con RP en col 10
    let startRow = 15;
    ...
    ws.eachRow((row, rowNum) => {
      ...
      console.log(`[completar-18m] ${rpStr} → animal ${animal.rp} (id=${animal.id}, nac=${animal.fecha_nac})`);
      
      if (animal.fecha_nac) {
        // Buscar pesada en rango 515-605 días
        ...
        if (p18) {
          console.log(`[completar-18m]   peso_18m=${p18.peso} fecha=${p18.fecha}`);
          // Peso 18m → col 11
          row.getCell(11).value = p18.peso;
          
          // CE de la misma fecha → col 14
          const ce = db.prepare("SELECT valor FROM mediciones WHERE animal_id = ? AND tipo = 'CE' AND fecha = ?").get(animal.id, p18.fecha);
          if (ce) {
            console.log(`[completar-18m]   ce=${ce.valor}`);
            row.getCell(14).value = ce.valor;
          }
        } else {
          console.log(`[completar-18m]   NO pesada en rango 515-605 dias`);
        }
      }
      ...
      // Padre RP → col 26 (solo si no tiene)
      if (!row.getCell(26).value && animal.padre_rp) {
        row.getCell(26).value = animal.padre_rp;
      }
      
      completados++;
    });
    
    console.log(`[completar-18m] Total: ${completados} completados, ${noEncontrados.length} no encontrados: ${noEncontrados.join(',')}`);
    
    const filename = `299_39100_18meses_completo.xlsx`;
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    await wb.xlsx.write(res);
    
  } catch(e) {
    console.error("Error completar INIA 18m:", e);
    res.status(500).json({ error: e.message });
  }
});
