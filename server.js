...
function ejecutarAccion(accion) {
  ...
  if (accion.accion === "compra_alimento") {
    ...
    // Búsqueda inteligente: exacto → parcial → primera palabra
    ...
    if (!al && nombre) {
      const palabra = nombre.split(/\s+/)[0];
      al = db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${palabra}%`);
    }
    if (!al) {
      // Listar los que existen para ayudar
