...
function ejecutarAccion(accion) {
  ...
  if (accion.accion === "registrar_sanidad") {
    ...
    let msg = `💉 Sanidad registrada!\n🏷️ RP ${animal.rp} | ${accion.tipo}\n💊 ${accion.producto || ""} | 📅 ${fecha}`;
    // Descontar del stock de IMPROLUX (que gestiona los números) si hay producto + dosis
    const dosisNum = parseFloat(accion.dosis);
    if (accion.producto && dosisNum > 0) {
      fetch(`${IMPROLUX_URL}/api/stock/aplicar`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ producto: accion.producto, cantidad: dosisNum, rp: animal.rp, fecha, detalle: accion.tipo })
      }).then(r => r.json()).then(d => {
        if (d && d.aviso) console.log("[STOCK IMPROLUX]", d.aviso);
        else if (d && d.error) console.log("[STOCK IMPROLUX] no descontó:", d.error);
      }).catch(e => console.error("[STOCK IMPROLUX] error:", e.message));
      msg += `\n📦 Descontado del stock: ${dosisNum} ${accion.producto}`;
    }
    return msg;
  }
...
app.get("/api/rodeo-resumen", (req, res) => {
  ...
  for (const a of animales) {
    ...
    else if (cs === 'RECRIA') cat = 'Recría';
    else if (cs === 'TERNERO') cat = (sx === 'HEMBRA') ? 'Terneras' : 'Terneros';
...
app.get("/", (req, res) => {
  ...
  res.json({ status: "GANADERÍA Bot activo 🐂", version: "5.2-stock-descuento", campos });
});
