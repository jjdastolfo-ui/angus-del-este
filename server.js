    ...
    try {
      ...
      if (arrMatch) {
        ...
        if (Array.isArray(arr)) {
          const resultados = [];
          for (const a of arr) { resultados.push(await ejecutarAccion(a)); }
          respuesta = resultados.join("\n\n");
        }
      } else {
        // Detectar múltiples JSON separados: {"accion":...} {"accion":...}
        const multiMatch = limpio.match(/\{[^{}]*"accion"[^{}]*\}/g);
        if (multiMatch && multiMatch.length > 1) {
          const resultados = [];
          for (const m of multiMatch) {
            try {
              const accion = JSON.parse(m);
              if (accion && accion.accion) resultados.push(await ejecutarAccion(accion));
            } catch {}
          }
          respuesta = resultados.join("\n\n");
        } else {
...
app.get("/api/animales/:rp", (req, res) => {
  ...
  if (!animal) animal = buscarAnimalTodos(req.params.rp); // also check vendidos
...
app.post("/api/importar/gallagher", (req, res) => {
  ...
  for (const r of registros) {
    try {
      ...
      // ── BUSCAR ANIMAL ──
      let animal = null;
      if (rp) animal = buscarAnimalTodos(rp);
      if (!animal && chip) {
        animal = db.prepare("SELECT * FROM animales WHERE chip = ? OR chip = ?").get(chip, chipRaw);
      }
      ...
      if (!animal && (rp || chip)) {
        ...
        try {
          db.prepare(`
            INSERT INTO animales (chip, rp, sexo, pelo, categoria, destino, estado, fecha_nac, fecha_ingreso, notas)
            VALUES (?, ?, ?, ?, 'RECRIA', 'PLANTEL', 'ACTIVO', ?, ?, ?)
          `).run(chip||null, nuevoRp, sexo, pelo, fechaNac, fecha, `Creado Gallagher: ${nombre_sesion||''}`);
      ...
      if (!animal) {
        resumen.errores++;
        resumen.no_encontrados.push(rp || `chip:${chip||'?'}`);
        continue;
      }
      ...
      if (peso && peso > 0) {
        db.prepare("INSERT INTO pesadas (animal_id, fecha, peso, contexto, gdp, notas) VALUES (?,?,?,'GALLAGHER',?,?)")
          .run(animal.id, fecha, peso, gdpGral||gdpProm||null, `${nombre_sesion||'Gallagher'}${gdpProm?` GDP_P:${gdpProm}`:''}${gdpGral?` GDP_G:${gdpGral}`:''}`);
        resumen.pesadas++;
      } else if (gdpProm || gdpGral) {
        // GDP sin peso → medición
      ...
      if (r.ce != null && r.ce !== '') {
        db.prepare("INSERT INTO mediciones (animal_id, fecha, tipo, valor, notas) VALUES (?,?,'CE',?,?)")
          .run(animal.id, fecha, parseFloat(r.ce), nombre_sesion||'Gallagher');
        resumen.mediciones++;
      }

      // ── ALTURA ──
      if (r.altura != null && r.altura !== '') {
        db.prepare("INSERT INTO mediciones (animal_id, fecha, tipo, valor, notas) VALUES (?,?,'ALTURA',?,?)")
          .run(animal.id, fecha, parseFloat(r.altura), nombre_sesion||'Gallagher');
        resumen.mediciones++;
      }

      // ── CARTEL (frame score) ──
      if (r.cartel != null && r.cartel !== '') {
        db.prepare("INSERT INTO mediciones (animal_id, fecha, tipo, valor, notas) VALUES (?,?,'FRAME',?,?)")
          .run(animal.id, fecha, parseFloat(r.cartel), nombre_sesion||'Gallagher');
        resumen.mediciones++;
      }

      // ── CONDICIÓN CORPORAL ──
      if (r.condicion != null && r.condicion !== '') {
        db.prepare("INSERT INTO mediciones (animal_id, fecha, tipo, valor, notas) VALUES (?,?,'CC',?,?)")
          .run(animal.id, fecha, parseFloat(r.condicion), nombre_sesion||'Gallagher');
        resumen.mediciones++;
      }
