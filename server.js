...
app.post("/api/informe-inia/completar-18meses", express.raw({ type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', limit: '10mb' }), async (req, res) => {
  ...
  try {
    ...
    // Datos empiezan en fila 15 (headers en fila 14)
    ws.eachRow((row, rowNum) => {
      if (rowNum < 15) return;
      
      const rp = row.getCell(10).value; // Col J = 10 = RP del toro/vaquillona
      if (!rp) return;
      ...
      // Buscar peso y CE en rango 18 meses (515-605 días)
      if (animal.fecha_nac) {
        // Peso 18m (col K = 11)
        if (!row.getCell(11).value) {
          const p18 = db.prepare(`
            SELECT peso FROM pesadas WHERE animal_id = ? 
          ...
          if (p18) {
            ...
            // CE de la misma fecha (col N = 14)
            if (!row.getCell(14).value) {
              const pesaFecha = db.prepare(`
                SELECT fecha FROM pesadas WHERE animal_id = ? 
                AND CAST((julianday(fecha) - julianday(?)) AS INTEGER) BETWEEN 515 AND 605
                ORDER BY fecha DESC LIMIT 1
              `).get(animal.id, animal.fecha_nac);
              if (pesaFecha) {
                const ce = db.prepare("SELECT valor FROM mediciones WHERE animal_id = ? AND tipo = 'CE' AND fecha = ?").get(animal.id, pesaFecha.fecha);
                if (ce) row.getCell(14).value = ce.valor;
              }
            }
          }
        }
      ...
      // Manejo (col Q = 17)
      if (!row.getCell(17).value) {
        row.getCell(17).value = manejo;
      }
      
      // Lote (col S = 19)
      const lote = getLoteAnimal(animal.id);
      if (lote && !row.getCell(19).value) {
        row.getCell(19).value = lote;
      }
      
      // Padre RP (col Z = 26) — solo si no tiene
