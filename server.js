...
async function procesarMensajeAsync(campoDb, campoKey, usuario, body, from, to) {
  ...
  if (esConsultaLluvia(body)) {
    ...
    await enviarWhatsApp(from, to, r, campoKey);
    return;
  }
  {
...
app.post("/webhook-interno", async (req, res) => {
  try {
    ...
    console.log("IA RAW:", rawRespuesta.substring(0, 500));
    ...
    try {
      ...
      if (arrMatch) {
        ...
        if (accionesFiltradas.length > 1) {
          const resultados = [];
          for (const a of accionesFiltradas) resultados.push(await ejecutarAccionAsync(a));
          respuesta = resultados.join("\n\n");
        } else if (accionesFiltradas.length === 1) {
          respuesta = await ejecutarAccionAsync(accionesFiltradas[0]);
