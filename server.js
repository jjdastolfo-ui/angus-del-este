...
app.post("/webhook-interno", async (req, res) => {
  try {
    ...
    if (fallbackNeeded) {
      ...
      // Extraer lista de RPs: buscar después de "RP:" o "RP " o "a los RP"
      const rpListMatch = body.match(/(?:a\s+los\s+)?RP[:\s]+(.+?)$/i);
      const rpList = rpListMatch ? rpListMatch[1].split(/[,\s]+/).map(r => r.trim()).filter(r => r && r.match(/^[A-Za-z0-9]+$/)) : [];

      // Detectar si es servicio → NO es sanidad
      const esServicio = /servicio|iatf|toro\s+repaso|inseminaci/i.test(body);

      // Detectar sanidad masiva (pero NO si dice "servicio")
      const esSanidad = /(?:vacun|apliqu|aplicar|desparasit)/i.test(body) || 
                         (/registrar/i.test(body) && !esServicio && /(?:para|a)\s+(?:los\s+)?RP/i.test(body));
      if (esSanidad && rpList.length > 0 && !esServicio) {
      ...
      // Detectar registro de servicio
      if (esServicio && !esSanidad && !esBaja) {
        // "Registrar servicio RP S403, temporada 2025, IATF GUAMINI fecha 2025-12-06, toro repaso ADE277 desde 2025-12-15, CC 5"
        const rpMatch = body.match(/RP\s+([A-Za-z0-9]+)/i);
        const tempMatch = body.match(/temporada\s+(\d{4})/i);
        const iatfMatch = body.match(/IATF\s+([A-Za-z0-9\s]+?)(?:\s+fecha|\s*$|,)/i);
        const fechaIatfMatch = body.match(/(?:IATF.*?fecha|fecha\s+IATF)\s+(\d{4}-\d{2}-\d{2}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})/i);
        const repasoMatch = body.match(/(?:toro\s+)?repaso\s+([A-Za-z0-9]+)/i);
        const fechaRepasoMatch = body.match(/(?:repaso.*?desde|desde)\s+(\d{4}-\d{2}-\d{2}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})/i);
        const ccMatch = body.match(/CC\s+(\d+\.?\d*)/i);

        if (rpMatch) {
          const animal = buscarAnimal(rpMatch[1]);
          if (animal) {
            const temporada = tempMatch ? tempMatch[1] : new Date().getFullYear().toString();
            const semenIatf = iatfMatch ? iatfMatch[1].trim() : null;
            let fechaIatf = fechaIatfMatch ? fechaIatfMatch[1] : null;
            const toroNat = repasoMatch ? repasoMatch[1] : null;
            let fechaToro = fechaRepasoMatch ? fechaRepasoMatch[1] : null;
            const ccPre = ccMatch ? parseFloat(ccMatch[1]) : null;

            // Normalizar fechas
            if (fechaIatf && !fechaIatf.match(/^\d{4}/)) {
              const [d,m,y] = fechaIatf.split(/[\/\-]/);
              fechaIatf = `${y.length===2?'20'+y:y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}`;
            }
            if (fechaToro && !fechaToro.match(/^\d{4}/)) {
              const [d,m,y] = fechaToro.split(/[\/\-]/);
              fechaToro = `${y.length===2?'20'+y:y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}`;
            }
            ...
            db.prepare(`
              ...
              VALUES (?, ?, 'IATF', ?, ?, ?, ?, ?, 'Registro manual')
            `).run(animal.id, temporada, semenIatf, fechaIatf, toroNat, fechaToro, ccPre);

            respuesta = `✅ Servicio registrado!\n🏷️ RP ${animal.rp} | Temporada ${temporada}`;
            if (semenIatf) respuesta += `\n🧬 IATF: ${semenIatf}${fechaIatf ? ` (${fechaIatf})` : ''}`;
            if (toroNat) respuesta += `\n🐂 Repaso: ${toroNat}${fechaToro ? ` (desde ${fechaToro})` : ''}`;
            if (ccPre) respuesta += `\n📊 CC: ${ccPre}`;
          } else {
            respuesta = `❌ No encontré animal con RP "${rpMatch[1]}"`;
          }
        }
      }
    }
