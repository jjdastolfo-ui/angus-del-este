...
const SCHEMA_SQL = `
  ...
  -- ── ENVÍOS A EVALUACIÓN GENÉTICA (INIA / BREEDPLAN) ──
  -- Cabecera: la "entrega" como evento por organismo + tipo + temporada
  CREATE TABLE IF NOT EXISTS envios_evaluacion (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    organismo TEXT NOT NULL DEFAULT 'INIA',   -- INIA | BREEDPLAN
    tipo TEXT NOT NULL,                        -- NACIMIENTO | DESTETE | 18MESES | SERVICIO
    temporada TEXT,                            -- '2024' o zafra '2024/2025'
    fecha_envio TEXT NOT NULL,
    archivo TEXT,                              -- nombre/ref de la planilla enviada
    estado TEXT DEFAULT 'ENVIADO',             -- BORRADOR | ENVIADO | CONFIRMADO | OBSERVADO | CORREGIDO
    notas TEXT,
    created_at TEXT DEFAULT (datetime('now'))
  );
  CREATE INDEX IF NOT EXISTS idx_envios_org ON envios_evaluacion(organismo);
  CREATE INDEX IF NOT EXISTS idx_envios_tipo ON envios_evaluacion(tipo);
  CREATE INDEX IF NOT EXISTS idx_envios_temp ON envios_evaluacion(temporada);

  -- Detalle: qué animales fueron en cada envío (estado por animal)
  CREATE TABLE IF NOT EXISTS envio_animales (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    envio_id INTEGER NOT NULL,
    animal_id INTEGER NOT NULL,
    estado TEXT DEFAULT 'ENVIADO',             -- ENVIADO | OBSERVADO | CORREGIDO | RECHAZADO | CONFIRMADO
    observacion TEXT,
    created_at TEXT DEFAULT (datetime('now')),
    FOREIGN KEY (envio_id) REFERENCES envios_evaluacion(id),
    FOREIGN KEY (animal_id) REFERENCES animales(id),
    UNIQUE(envio_id, animal_id)
  );
  CREATE INDEX IF NOT EXISTS idx_envio_animales_envio ON envio_animales(envio_id);
  CREATE INDEX IF NOT EXISTS idx_envio_animales_animal ON envio_animales(animal_id);

  -- Bitácora de seguimiento y correcciones (timeline)
  CREATE TABLE IF NOT EXISTS envio_eventos (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    envio_id INTEGER NOT NULL,
    animal_id INTEGER,                         -- null = aplica a todo el envío
    fecha TEXT NOT NULL,
    estado TEXT,                               -- estado al que pasa
    detalle TEXT,
    created_at TEXT DEFAULT (datetime('now')),
    FOREIGN KEY (envio_id) REFERENCES envios_evaluacion(id)
  );
  CREATE INDEX IF NOT EXISTS idx_envio_eventos_envio ON envio_eventos(envio_id);
`;
...
function buscarAnimalTodos(identificador) {
  if (!identificador) return null;
  const id = String(identificador).trim();
  // Primero buscar por RP exacto
  let animal = db.prepare("SELECT * FROM animales WHERE LOWER(rp) = LOWER(?)").get(id);
  if (animal) return animal;
  // Buscar sin prefijo ADE
  if (id.toUpperCase().startsWith('ADE')) {
    animal = db.prepare("SELECT * FROM animales WHERE LOWER(rp) = LOWER(?)").get(id.substring(3));
    if (animal) return animal;
  }
  // Buscar con prefijo ADE
  animal = db.prepare("SELECT * FROM animales WHERE LOWER(rp) = LOWER(?)").get('ADE' + id);
  if (animal) return animal;
  // Buscar por chip
  animal = buscarAnimalPorChip(id, false);
  if (animal) return animal;
...
{"accion":"resultado_tacto","rp":"","resultado":"PREÑADA_IATF/PREÑADA_TORO/VACIA","fecha_tacto":"YYYY-MM-DD","temporada":"2025"}
...
{"accion":"consulta","tipo":"TIPO","temporada":"2025","anio":"2025","filtros":{}}

IMPORTANTE sobre temporada vs anio:
- "temporada" = temporada de servicio (ej: servicio 2025, tacto 2025). Se usa para servicios y tactos.
- "anio" = año calendario de la fecha del evento. Se usa para pesadas, pariciones, destetes.
- "terneros nacidos en 2025" → anio 2025 (filtra por fecha_nac o fecha_parto del 2025)
- "destetes 2026" → anio 2026 (filtra pesadas de destete del 2026)
- "servicios 2025" → temporada 2025
- "% preñez temporada 2025" → temporada 2025
...
- prenadas_hoy → vacas ACTIVAS cuyo ÚLTIMO servicio es preñada (estado actual, no histórico)
- tacto_resumen → % preñez del último servicio de cada vaca (sin duplicados), % IATF, % toro, vacías
...
Filtros opcionales en "filtros": temporada, anio, toro, categoria, sexo, registro, lote, fecha_desde, fecha_hasta, contexto_pesada, producto

{"accion":"texto","mensaje":"respuesta"}

ACCIONES DE STOCK Y COSTOS:
{"accion":"crear_insumo","nombre":"Ivermectina","tipo":"VETERINARIO","unidad":"dosis","stock_actual":50,"stock_minimo":10,"costo_unitario":2,"proveedor":"Zambrano"}
{"accion":"movimiento_insumo","nombre":"Ivermectina","tipo":"ENTRADA/SALIDA","cantidad":20,"detalle":"Compra"}
{"accion":"crear_alimento","nombre":"Ración Toros","tipo":"RACION","stock_actual":500,"costo_por_kg":0.5}
{"accion":"compra_alimento","nombre":"Ración Toros","cantidad":1000,"costo_total":500,"detalle":"Compra Mercado Rural"}
{"accion":"programar_consumo","nombre":"Ración Toros","lote":"TOROS 1","cantidad_animales":10,"kg_por_animal_dia":3,"fecha_inicio":"YYYY-MM-DD","fecha_fin":"YYYY-MM-DD"}
{"accion":"descontar_consumo_diario"}
{"accion":"eliminar_consumo","lote":"TOROS 1"}
{"accion":"crear_genetica","tipo":"PAJUELA/EMBRION","toro_nombre":"Lucuma","toro_rp":"4178","donante_nombre":"","donante_rp":"","cantidad":10,"costo_unitario":50}
{"accion":"usar_genetica","nombre":"Lucuma","cantidad":1}
{"accion":"registrar_costo","rp":"S219","concepto":"ALIMENTACION/SANIDAD/SERVICIO/MANTENIMIENTO/FLETE/OTROS","monto":50,"detalle":"detalle","fecha":"YYYY-MM-DD"}
{"accion":"crear_costo_fijo","concepto":"ALQUILER/SRA/BREEDPLAN/BPS/SEGURO","monto_mensual":500,"notas":""}
{"accion":"costo_masivo","rps":["S219","S220"],"concepto":"SANIDAD","monto":5,"detalle":"Vacuna aftosa"}
{"accion":"ver_stock"}
{"accion":"ver_alertas"}
{"accion":"ver_productividad"}
{"accion":"ver_costos","rp":"S219"}
{"accion":"registrar_evento","rp":"2077","descripcion":"Se lastimó pata delantera derecha","tipo":"OBSERVACION/SANIDAD/REPRODUCTIVO/COMPORTAMIENTO/MANEJO"}
{"accion":"ver_eventos","rp":"2077"}

VOCABULARIO DE STOCK:
- "compré/ingresó/llegaron X dosis de Y" → movimiento_insumo ENTRADA
- "usé/apliqué/gasté X dosis de Y" → movimiento_insumo SALIDA
- "compré X kg de ración/alimento" → compra_alimento
- "los toros comen X kg por día" → programar_consumo
- "descontar consumo de hoy" → descontar_consumo_diario
- "quedan X pajuelas de Y" / "tengo X embriones de Z" → crear_genetica
- "usé 1 pajuela de Y" → usar_genetica
- "cuánto stock tengo" / "stock" → ver_stock
- "alertas" / "qué tengo pendiente" → ver_alertas
- "productividad" / "ranking toros" / "eficiencia" → ver_productividad
- "cuánto costó el S219" / "costos de S219" → ver_costos
- "alquiler $500 por mes" / "el alquiler es $500" → crear_costo_fijo con concepto ALQUILER
- "SRA $20 por mes" / "breedplan $15 por mes" → crear_costo_fijo
- "repartir costos" → repartir costos fijos del mes entre animales
- "costos fijos" / "costos campo" → ver costos fijos
- "la 2077 se lastimó" / "toro saltó 2077" / "evento: RP tal cosa" → registrar_evento
- "eventos de 2077" / "historial de 2077" → ver_eventos
...
- TACTO CONFIRMA SERVICIO: el tacto se vincula automáticamente al último servicio pendiente de la vaca. Si dice "tacto 2026" o "temporada 2025" → incluir "temporada" en el JSON para vincular al servicio correcto. Si no dice temporada, se busca el último servicio sin resultado.
- "Tacto S219 preñada de IATF temporada 2025" → resultado_tacto con rp S219, resultado PREÑADA_IATF, temporada 2025
- "S219 vacía" → resultado_tacto con resultado VACIA (busca último servicio sin resultado)
...
- SERVICIO: solo hembras de 12+ meses pueden entrar en servicio (vacas, vaquillonas). Las terneras menores de 12 meses NO se sirven.
- Cuando el usuario hace una PREGUNTA sobre datos (no un registro) → usar accion "consulta" con el tipo apropiado.
- "¿Cuántas vacas preñadas hay?" / "vacas preñadas hoy" / "listado de preñadas" → consulta tipo prenadas_hoy (estado actual, no histórico)
...
Stock insumos: ${JSON.stringify((() => { try { return db.prepare("SELECT nombre,stock_actual,unidad FROM stock_insumos WHERE stock_actual>0").all(); } catch(e) { return []; } })())}
Stock alimento: ${JSON.stringify((() => { try { return db.prepare("SELECT nombre,stock_actual FROM stock_alimento").all(); } catch(e) { return []; } })())}
Stock genética: ${JSON.stringify((() => { try { return db.prepare("SELECT tipo,toro_nombre,donante_nombre,cantidad FROM stock_genetica WHERE cantidad>0").all(); } catch(e) { return []; } })())}`;
}
...
function ejecutarAccion(accion) {
  ...
  // ── STOCK INSUMOS ──
  if (accion.accion === "crear_insumo") {
    const { nombre, tipo, unidad, stock_actual, stock_minimo, costo_unitario, proveedor } = accion;
    if (!nombre) return "❌ Falta nombre del insumo.";
    db.prepare("INSERT INTO stock_insumos (nombre,tipo,unidad,stock_actual,stock_minimo,costo_unitario,proveedor) VALUES (?,?,?,?,?,?,?)")
      .run(nombre, tipo||'VETERINARIO', unidad||'dosis', stock_actual||0, stock_minimo||0, costo_unitario||0, proveedor||'');
    return `✅ Insumo creado: ${nombre}\n📦 Stock: ${stock_actual||0} ${unidad||'dosis'}${stock_minimo ? `\n⚠️ Alerta cuando baje de ${stock_minimo}` : ''}`;
  }

  if (accion.accion === "movimiento_insumo") {
    const { nombre, tipo, cantidad, detalle } = accion;
    const insumo = db.prepare("SELECT * FROM stock_insumos WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${nombre}%`);
    if (!insumo) return `❌ No encontré insumo "${nombre}". Usá "ver stock" para ver los disponibles.`;
    const signo = tipo === 'ENTRADA' ? 1 : -1;
    db.prepare("INSERT INTO stock_movimientos (stock_id,fecha,tipo,cantidad,detalle) VALUES (?,?,?,?,?)")
      .run(insumo.id, hoy, tipo, cantidad, detalle||'');
    db.prepare("UPDATE stock_insumos SET stock_actual = stock_actual + ? WHERE id = ?").run(signo * cantidad, insumo.id);
    const actualizado = db.prepare("SELECT stock_actual FROM stock_insumos WHERE id = ?").get(insumo.id);
    return `✅ ${tipo}: ${cantidad} ${insumo.unidad} de ${insumo.nombre}\n📦 Stock actual: ${actualizado.stock_actual} ${insumo.unidad}`;
  }

  // ── STOCK ALIMENTO ──
  if (accion.accion === "crear_alimento") {
    const { nombre, tipo, stock_actual, costo_por_kg } = accion;
    if (!nombre) return "❌ Falta nombre del alimento.";
    db.prepare("INSERT INTO stock_alimento (nombre,tipo,stock_actual,costo_por_kg) VALUES (?,?,?,?)")
      .run(nombre, tipo||'RACION', stock_actual||0, costo_por_kg||0);
    return `✅ Alimento creado: ${nombre}\n📦 Stock: ${stock_actual||0} kg`;
  }

  if (accion.accion === "compra_alimento") {
    const { nombre, cantidad, costo_total, detalle } = accion;
    const al = db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${nombre}%`);
    if (!al) return `❌ No encontré alimento "${nombre}".`;
    db.prepare("INSERT INTO alimento_movimientos (alimento_id,fecha,tipo,cantidad,costo_total,detalle) VALUES (?,?,'COMPRA',?,?,?)")
      .run(al.id, hoy, cantidad, costo_total||0, detalle||'');
    db.prepare("UPDATE stock_alimento SET stock_actual = stock_actual + ? WHERE id = ?").run(cantidad, al.id);
    if (costo_total && cantidad) db.prepare("UPDATE stock_alimento SET costo_por_kg = ? WHERE id = ?").run(costo_total/cantidad, al.id);
    const act = db.prepare("SELECT stock_actual FROM stock_alimento WHERE id = ?").get(al.id);
    return `✅ Compra: ${cantidad}kg de ${al.nombre}\n💰 $${costo_total||0}\n📦 Stock: ${act.stock_actual}kg`;
  }

  if (accion.accion === "programar_consumo") {
    const { nombre, lote, cantidad_animales, kg_por_animal_dia, fecha_inicio, fecha_fin } = accion;
    const al = db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${nombre}%`);
    if (!al) return `❌ No encontré alimento "${nombre}".`;
    
    // Anti-duplicado: si ya existe un consumo activo para este alimento + lote → actualizar
    const existe = db.prepare("SELECT id FROM consumos_programados WHERE alimento_id = ? AND LOWER(lote_nombre) = LOWER(?) AND activo = 1")
      .get(al.id, lote || '');
    if (existe) {
      db.prepare("UPDATE consumos_programados SET cantidad_animales=?, kg_por_animal_dia=?, fecha_inicio=COALESCE(?,fecha_inicio), fecha_fin=? WHERE id=?")
        .run(cantidad_animales||1, kg_por_animal_dia, fecha_inicio||null, fecha_fin||null, existe.id);
    } else {
      db.prepare("INSERT INTO consumos_programados (alimento_id,lote_nombre,cantidad_animales,kg_por_animal_dia,fecha_inicio,fecha_fin) VALUES (?,?,?,?,?,?)")
        .run(al.id, lote||'', cantidad_animales||1, kg_por_animal_dia, fecha_inicio||hoy, fecha_fin||null);
    }
    const consumoDia = (cantidad_animales||1) * kg_por_animal_dia;
    const totalConsumo = db.prepare("SELECT SUM(cantidad_animales * kg_por_animal_dia) as total FROM consumos_programados WHERE alimento_id = ? AND activo = 1").get(al.id);
    const dias = (totalConsumo?.total > 0 && al.stock_actual > 0) ? Math.floor(al.stock_actual / totalConsumo.total) : 0;
    return `✅ Consumo ${existe ? 'actualizado' : 'programado'}: ${cantidad_animales} animales × ${kg_por_animal_dia}kg/día = ${consumoDia}kg/día\n📦 Stock: ${al.stock_actual}kg → alcanza para ${dias} días`;
  }

  if (accion.accion === "eliminar_consumo") {
    const { nombre, lote } = accion;
    if (lote) {
      db.prepare("UPDATE consumos_programados SET activo = 0 WHERE LOWER(lote_nombre) = LOWER(?) AND activo = 1").run(lote);
      return `✅ Consumo del lote "${lote}" desactivado.`;
    }
    if (nombre) {
      const al = db.prepare("SELECT id FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${nombre}%`);
      if (al) { db.prepare("UPDATE consumos_programados SET activo = 0 WHERE alimento_id = ? AND activo = 1").run(al.id); return `✅ Todos los consumos de "${nombre}" desactivados.`; }
    }
    return `❌ Indicá el lote o alimento a desactivar.`;
  }

  if (accion.accion === "descontar_consumo_diario") {
    const consumos = db.prepare("SELECT * FROM consumos_programados WHERE activo = 1 AND fecha_inicio <= ? AND (fecha_fin IS NULL OR fecha_fin >= ?)").all(hoy, hoy);
    let total = 0, detalles = [], animalesCosto = 0;
    for (const c of consumos) {
      const consumoDia = c.cantidad_animales * c.kg_por_animal_dia;
      db.prepare("UPDATE stock_alimento SET stock_actual = MAX(0, stock_actual - ?) WHERE id = ?").run(consumoDia, c.alimento_id);
      db.prepare("INSERT INTO alimento_movimientos (alimento_id,fecha,tipo,cantidad,detalle) VALUES (?,?,'CONSUMO',?,?)")
        .run(c.alimento_id, hoy, consumoDia, `${c.lote_nombre}: ${c.cantidad_animales}×${c.kg_por_animal_dia}kg`);
      total += consumoDia;
      const al = db.prepare("SELECT nombre,stock_actual,costo_por_kg FROM stock_alimento WHERE id = ?").get(c.alimento_id);
      const costoPorAnimal = c.kg_por_animal_dia * (al.costo_por_kg || 0);
      
      // Registrar costo a cada animal del lote
      if (costoPorAnimal > 0 && c.lote_nombre) {
        const lote = db.prepare("SELECT id FROM lotes WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${c.lote_nombre}%`);
        if (lote) {
          const animalesLote = db.prepare("SELECT la.animal_id FROM lote_animales la JOIN animales a ON a.id = la.animal_id WHERE la.lote_id = ? AND a.estado = 'ACTIVO'").all(lote.id);
          for (const a of animalesLote) {
            db.prepare("INSERT INTO costos (animal_id,fecha,concepto,detalle,monto) VALUES (?,?,'ALIMENTACION',?,?)")
              .run(a.animal_id, hoy, `${al.nombre} ${c.kg_por_animal_dia}kg`, costoPorAnimal);
            animalesCosto++;
          }
        }
      }
      
      detalles.push(`  ${al.nombre} → -${consumoDia}kg (quedan ${al.stock_actual.toFixed(0)}kg)${costoPorAnimal > 0 ? ` · $${costoPorAnimal.toFixed(2)}/animal` : ''}`);
    }
    let resp = `✅ Consumo del día descontado: ${total.toFixed(1)}kg\n${detalles.join('\n')}`;
    if (animalesCosto > 0) resp += `\n💰 Costo registrado a ${animalesCosto} animales`;
    return resp;
  }
  ...
  if (accion.accion === "ver_stock") {
    let resp = "📦 *STOCK ACTUAL*\n";
    const insumos = db.prepare("SELECT * FROM stock_insumos WHERE stock_actual > 0 ORDER BY nombre").all();
    if (insumos.length) {
      resp += "\n💊 *Insumos veterinarios:*\n";
      insumos.forEach(i => resp += `  ${i.nombre}: ${i.stock_actual} ${i.unidad}${i.stock_actual <= i.stock_minimo ? ' ⚠️ BAJO' : ''}\n`);
    }
    const alimentos = db.prepare("SELECT * FROM stock_alimento ORDER BY nombre").all();
    const consumos = db.prepare("SELECT * FROM consumos_programados WHERE activo = 1").all();
    if (alimentos.length) {
      resp += "\n🌾 *Alimento:*\n";
      alimentos.forEach(a => {
        const csDia = consumos.filter(c => c.alimento_id === a.id).reduce((s, c) => s + c.cantidad_animales * c.kg_por_animal_dia, 0);
        const dias = csDia > 0 ? Math.floor(a.stock_actual / csDia) : '∞';
        resp += `  ${a.nombre}: ${a.stock_actual}kg${csDia > 0 ? ` (${csDia}kg/día → ${dias} días)` : ''}\n`;
      });
    }
    ...
    if (!insumos.length && !alimentos.length && !genetica.length) resp += "\nNo hay stock cargado.";
    return resp;
  }
  ...
  if (accion.accion === "ver_alertas") {
    ...
    // Stock bajo
    const stockBajo = db.prepare("SELECT * FROM stock_insumos WHERE stock_actual <= stock_minimo AND stock_minimo > 0").all();
    stockBajo.forEach(s => alertas.push(`⚠️ ${s.nombre}: ${s.stock_actual} ${s.unidad} (mín: ${s.stock_minimo})`));
    // Alimento
    const als = db.prepare("SELECT * FROM stock_alimento").all();
    const css = db.prepare("SELECT * FROM consumos_programados WHERE activo=1").all();
    als.forEach(a => {
      const cd = css.filter(c=>c.alimento_id===a.id).reduce((s,c)=>s+c.cantidad_animales*c.kg_por_animal_dia,0);
      if(cd>0){const d=Math.floor(a.stock_actual/cd);if(d<7)alertas.push(`${d<3?'🚨':'⚠️'} ${a.nombre}: ${d} días de stock (${a.stock_actual.toFixed(0)}kg)`);}
    });
  ...
  if (accion.accion === "ver_productividad") {
    ...
    const gdp = db.prepare(`SELECT a.padre_rp as padre, COUNT(*) as n, AVG(p.gdp) as gdp
      FROM animales a JOIN pesadas p ON p.animal_id=a.id WHERE a.padre_rp IS NOT NULL AND a.padre_rp!='' AND p.gdp>0
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    const historial = getHistorial(campoDb, usuario);
    ...
    // Truncar a 1600 chars (límite WhatsApp)
    if (respuesta.length > 1500) respuesta = respuesta.substring(0, 1500) + "\n\n... (respuesta truncada)";
    twiml.message(respuesta || "No entendí. Intentá de nuevo.");
  } catch (err) {
...
app.post("/webhook-interno", async (req, res) => {
  try {
    ...
    if (/^descontar\s+consumo/i.test(body)) {
      const result = ejecutarAccion({ accion: 'descontar_consumo_diario' });
      return res.json({ respuesta: result });
    }
    if (/^(arreglar|fix|reparar)\s+consumos?/i.test(body) || bodyLower === 'arreglar consumos') {
      // Quitar fecha_fin de todos los consumos activos para que sean permanentes
      db.prepare("UPDATE consumos_programados SET fecha_fin = NULL WHERE activo = 1").run();
      return res.json({ respuesta: "✅ Consumos arreglados — ahora son permanentes (sin fecha fin). Descontá con 'descontar consumo'." });
    }
    if (/^ver\s+consumos?/i.test(body) || bodyLower === 'consumos') {
      const consumos = db.prepare("SELECT * FROM consumos_programados WHERE activo = 1").all();
      if (!consumos.length) return res.json({ respuesta: "📋 No hay consumos programados activos." });
      let resp = "📋 *Consumos programados activos:*\n\n";
      let totalDia = 0;
      consumos.forEach(c => {
        const kg = c.cantidad_animales * c.kg_por_animal_dia;
        totalDia += kg;
        resp += `  ${c.lote_nombre}: ${c.cantidad_animales} anim × ${c.kg_por_animal_dia}kg = ${kg.toFixed(1)}kg/día${c.fecha_fin ? ` (hasta ${c.fecha_fin})` : ' (permanente)'}\n`;
      });
      resp += `\n📊 Total: ${totalDia.toFixed(1)}kg/día`;
      return res.json({ respuesta: resp });
    }
...
app.get("/api/animales/:rp", (req, res) => {
  ...
  try {
    envios = db.prepare(`
      SELECT e.id, e.organismo, e.tipo, e.temporada, e.fecha_envio, e.archivo,
             e.estado AS estado_envio, ea.id AS ea_id, ea.estado AS estado_animal, ea.observacion
      FROM envio_animales ea
      JOIN envios_evaluacion e ON e.id = ea.envio_id
      WHERE ea.animal_id = ?
      ORDER BY e.fecha_envio DESC, e.created_at DESC
    `).all(animal.id);
  } catch(e) {}
  res.json({ ...animal, pesadas, mediciones, ecografias, servicios, sanidad, hijos, hijos_padre, lote, eventos, envios });
});
...
// ── INFORMES PDF ─────────────────────────────────────────────────────────────
// ── API CONSUMOS PROGRAMADOS ──
app.get("/api/consumos", (req, res) => {
  const campo = req.query.campo || CAMPO_DEFAULT;
  const d = getDB(campo);
  const rows = d.prepare(`
    SELECT cp.*, sa.nombre as alimento_nombre FROM consumos_programados cp
    JOIN stock_alimento sa ON sa.id = cp.alimento_id ORDER BY cp.activo DESC, cp.lote_nombre
  `).all();
  res.json(rows);
});
...
app.delete("/api/consumos/:id", (req, res) => {
  const campo = req.query.campo || CAMPO_DEFAULT;
  const d = getDB(campo);
  d.prepare("DELETE FROM consumos_programados WHERE id = ?").run(req.params.id);
  res.json({ mensaje: `✅ Consumo #${req.params.id} eliminado` });
});
...
// ══════════════════════════════════════════════════════════════════════════════
// ── EXPORTAR CSV (servicios / pesadas / registros) ────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
function toCSV(headers, rows, sepParam) {
  const sep = sepParam === 'coma' ? ',' : ';';
  const esc = v => {
  ...
  for (const r of rows) lines.push(r.map(esc).join(sep));
...
// Servicios (con RP) — más reciente primero
app.get("/api/export/servicios.csv", (req, res) => {
  try {
    const rows = db.prepare(`
      SELECT a.rp, a.categoria, s.temporada, s.tipo_servicio, s.semen_iatf, s.fecha_iatf,
             s.toro_natural, s.fecha_ingreso_toro, s.tacto_pre, s.cc_pre, s.tacto_servicio,
             s.resultado, s.fecha_parto, s.ternero_rp, s.peso_nacimiento, s.peso_destete, s.sexo_cria, s.notas
      FROM servicios s JOIN animales a ON a.id = s.animal_id
      ORDER BY s.temporada DESC, a.rp
    `).all();
    const headers = ['RP','Categoria','Temporada','Tipo Servicio','Semen IATF','Fecha IATF','Toro Natural','Fecha Ingreso Toro','Tacto Pre','CC Pre','Tacto Servicio','Resultado','Fecha Parto','Ternero RP','Peso Nac','Peso Destete','Sexo Cria','Notas'];
    const data = rows.map(r => [r.rp,r.categoria,r.temporada,r.tipo_servicio,r.semen_iatf,r.fecha_iatf,r.toro_natural,r.fecha_ingreso_toro,r.tacto_pre,r.cc_pre,r.tacto_servicio,r.resultado,r.fecha_parto,r.ternero_rp,r.peso_nacimiento,r.peso_destete,r.sexo_cria,r.notas]);
    sendCSV(res, `servicios_${hoyStr()}.csv`, toCSV(headers, data, req.query.sep));
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Pesadas (con RP) — últimas primero. ?limite=N opcional
app.get("/api/export/pesadas.csv", (req, res) => {
  try {
    const limite = parseInt(req.query.limite) || 5000;
    const rows = db.prepare(`
      SELECT a.rp, a.categoria, a.registro, p.fecha, p.peso, p.contexto, p.gdp, p.notas
      FROM pesadas p JOIN animales a ON a.id = p.animal_id
      ORDER BY p.fecha DESC, a.rp
      LIMIT ?
    `).all(limite);
    const headers = ['RP','Categoria','Registro','Fecha','Peso (kg)','Contexto','GDP (g/d)','Notas'];
    const data = rows.map(r => [r.rp, r.categoria, r.registro, r.fecha, r.peso, r.contexto, r.gdp != null ? Math.round(r.gdp*1000) : '', r.notas]);
    sendCSV(res, `pesadas_${hoyStr()}.csv`, toCSV(headers, data, req.query.sep));
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Últimos registros — actividad reciente unificada. ?limite=N (default 500)
app.get("/api/export/registros.csv", (req, res) => {
  try {
    const limite = parseInt(req.query.limite) || 500;
    const regs = [];
    db.prepare("SELECT a.rp, p.fecha, p.peso, p.contexto FROM pesadas p JOIN animales a ON a.id=p.animal_id").all()
      .forEach(r => regs.push({ fecha: r.fecha, rp: r.rp, tipo: 'PESADA', detalle: `${r.peso} kg${r.contexto ? ' ('+r.contexto+')' : ''}` }));
    db.prepare("SELECT a.rp, m.fecha, m.tipo, m.valor, m.valor_texto FROM mediciones m JOIN animales a ON a.id=m.animal_id").all()
      .forEach(r => regs.push({ fecha: r.fecha, rp: r.rp, tipo: 'MEDICION ' + r.tipo, detalle: String(r.valor != null ? r.valor : (r.valor_texto || '')) }));
    db.prepare("SELECT a.rp, s.fecha, s.tipo, s.producto FROM sanidad s JOIN animales a ON a.id=s.animal_id").all()
      .forEach(r => regs.push({ fecha: r.fecha, rp: r.rp, tipo: 'SANIDAD ' + (r.tipo || ''), detalle: r.producto || '' }));
    db.prepare("SELECT a.rp, s.fecha_iatf, s.fecha_ingreso_toro, s.created_at, s.temporada, s.tipo_servicio, s.resultado FROM servicios s JOIN animales a ON a.id=s.animal_id").all()
      .forEach(r => { const f = r.fecha_iatf || r.fecha_ingreso_toro || (r.created_at ? String(r.created_at).slice(0,10) : ''); regs.push({ fecha: f, rp: r.rp, tipo: 'SERVICIO', detalle: `${r.temporada || ''} ${r.tipo_servicio || ''} ${r.resultado || ''}`.trim() }); });
    try {
      db.prepare("SELECT a.rp, e.fecha, e.tipo, e.descripcion FROM eventos e JOIN animales a ON a.id=e.animal_id").all()
        .forEach(r => regs.push({ fecha: r.fecha, rp: r.rp, tipo: 'EVENTO ' + (r.tipo || ''), detalle: r.descripcion || '' }));
    } catch(e) {}
    regs.sort((a, b) => String(b.fecha || '').localeCompare(String(a.fecha || '')));
    const top = regs.slice(0, limite);
    const headers = ['Fecha','RP','Tipo','Detalle'];
    const data = top.map(r => [r.fecha, r.rp, r.tipo, r.detalle]);
    sendCSV(res, `registros_${hoyStr()}.csv`, toCSV(headers, data, req.query.sep));
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ── ESTADÍSTICAS ANUALES (para gráficos dashboard ADE) ──
app.get("/api/stats/anuales", (req, res) => {
  try {
    // Preñez por temporada
    const prenez = db.prepare(`
      SELECT temporada,
        COUNT(*) total,
        SUM(CASE WHEN UPPER(COALESCE(resultado,'')) LIKE '%PRE%' THEN 1 ELSE 0 END) prenadas,
        SUM(CASE WHEN UPPER(COALESCE(resultado,'')) LIKE '%VAC%' THEN 1 ELSE 0 END) vacias
      FROM servicios
      WHERE temporada IS NOT NULL AND TRIM(temporada) != ''
      GROUP BY temporada ORDER BY temporada
    `).all().map(r => {
      const diag = r.prenadas + r.vacias;
      return { temporada: r.temporada, total: r.total, prenadas: r.prenadas, vacias: r.vacias, pct: diag > 0 ? Math.round(r.prenadas * 100 / diag) : null };
    }).filter(r => r.pct !== null);

    // Peso al destete promedio por temporada
    const destete = db.prepare(`
      SELECT temporada, COUNT(*) n, ROUND(AVG(peso_destete),1) promedio
      FROM servicios
      WHERE peso_destete > 0 AND temporada IS NOT NULL AND TRIM(temporada) != ''
      GROUP BY temporada ORDER BY temporada
    `).all();

    // GDP de recrías/terneros por año (kg/día)
    const gdp = db.prepare(`
      SELECT substr(p.fecha,1,4) anio, COUNT(*) n, ROUND(AVG(p.gdp),3) promedio
      FROM pesadas p JOIN animales a ON a.id = p.animal_id
      WHERE p.gdp IS NOT NULL AND p.gdp > 0
        AND UPPER(COALESCE(a.categoria,'')) IN ('RECRIA','TERNERO','VAQUILLONA')
...
// ══════════════════════════════════════════════════════════════════════════════
// ── MÓDULO: STOCK INSUMOS VETERINARIOS ───────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════

app.get("/api/stock/insumos", (req, res) => {
  const rows = db.prepare("SELECT * FROM stock_insumos ORDER BY tipo, nombre").all();
  res.json(rows);
});

app.post("/api/stock/insumos", (req, res) => {
  const { nombre, tipo, unidad, stock_actual, stock_minimo, costo_unitario, proveedor, vencimiento, notas } = req.body;
  db.prepare("INSERT INTO stock_insumos (nombre,tipo,unidad,stock_actual,stock_minimo,costo_unitario,proveedor,vencimiento,notas) VALUES (?,?,?,?,?,?,?,?,?)")
    .run(nombre, tipo||'VETERINARIO', unidad||'dosis', stock_actual||0, stock_minimo||0, costo_unitario||0, proveedor||'', vencimiento||'', notas||'');
  res.json({ mensaje: "✅ Insumo creado" });
});

app.post("/api/stock/insumos/movimiento", (req, res) => {
  const { stock_id, tipo, cantidad, costo_total, detalle, animal_id, lote_id } = req.body;
  const fecha = req.body.fecha || new Date().toISOString().slice(0,10);
  db.prepare("INSERT INTO stock_movimientos (stock_id,fecha,tipo,cantidad,costo_total,detalle,animal_id,lote_id) VALUES (?,?,?,?,?,?,?,?)")
    .run(stock_id, fecha, tipo, cantidad, costo_total||0, detalle||'', animal_id||null, lote_id||null);
  // Actualizar stock
  const signo = tipo === 'ENTRADA' ? 1 : -1;
  db.prepare("UPDATE stock_insumos SET stock_actual = stock_actual + ? WHERE id = ?").run(signo * cantidad, stock_id);
  const insumo = db.prepare("SELECT * FROM stock_insumos WHERE id = ?").get(stock_id);
  res.json({ mensaje: `✅ ${tipo}: ${cantidad} ${insumo?.unidad||'u'} de ${insumo?.nombre||''}. Stock: ${insumo?.stock_actual||0}` });
});
...
// ══════════════════════════════════════════════════════════════════════════════
// ── MÓDULO: STOCK ALIMENTO CON CONSUMOS PROGRAMADOS ──────────────────────────
// ══════════════════════════════════════════════════════════════════════════════

app.get("/api/stock/alimento", (req, res) => {
  const alimentos = db.prepare("SELECT * FROM stock_alimento ORDER BY nombre").all();
  const consumos = db.prepare("SELECT * FROM consumos_programados WHERE activo = 1").all();
  // Calcular consumo diario total y días restantes para cada alimento
  const result = alimentos.map(a => {
    const csProg = consumos.filter(c => c.alimento_id === a.id);
    const consumoDiario = csProg.reduce((s, c) => s + (c.cantidad_animales * c.kg_por_animal_dia), 0);
    const diasRestantes = consumoDiario > 0 ? Math.floor(a.stock_actual / consumoDiario) : null;
    return { ...a, consumos: csProg, consumo_diario: consumoDiario, dias_restantes: diasRestantes };
  });
  res.json(result);
});

app.post("/api/stock/alimento", (req, res) => {
  const { nombre, tipo, unidad, stock_actual, costo_por_kg, proveedor, notas } = req.body;
  db.prepare("INSERT INTO stock_alimento (nombre,tipo,unidad,stock_actual,costo_por_kg,proveedor,notas) VALUES (?,?,?,?,?,?,?)")
    .run(nombre, tipo||'RACION', unidad||'kg', stock_actual||0, costo_por_kg||0, proveedor||'', notas||'');
  res.json({ mensaje: "✅ Alimento creado" });
});

app.delete("/api/stock/alimento/:id", (req, res) => {
  const id = parseInt(req.params.id);
  db.prepare("DELETE FROM consumos_programados WHERE alimento_id = ?").run(id);
  db.prepare("DELETE FROM alimento_movimientos WHERE alimento_id = ?").run(id);
  db.prepare("DELETE FROM stock_alimento WHERE id = ?").run(id);
  res.json({ mensaje: "✅ Alimento eliminado con consumos y movimientos" });
});

app.post("/api/stock/alimento/compra", (req, res) => {
  const { alimento_id, cantidad, costo_total, detalle } = req.body;
  const fecha = req.body.fecha || new Date().toISOString().slice(0,10);
  db.prepare("INSERT INTO alimento_movimientos (alimento_id,fecha,tipo,cantidad,costo_total,detalle) VALUES (?,?,'COMPRA',?,?,?)")
    .run(alimento_id, fecha, cantidad, costo_total||0, detalle||'');
  db.prepare("UPDATE stock_alimento SET stock_actual = stock_actual + ? WHERE id = ?").run(cantidad, alimento_id);
  // Actualizar costo/kg si viene
  if (costo_total && cantidad) {
    db.prepare("UPDATE stock_alimento SET costo_por_kg = ? WHERE id = ?").run(costo_total / cantidad, alimento_id);
  }
  const al = db.prepare("SELECT * FROM stock_alimento WHERE id = ?").get(alimento_id);
  res.json({ mensaje: `✅ Compra: ${cantidad}kg de ${al?.nombre}. Stock: ${al?.stock_actual}kg` });
});

app.post("/api/stock/alimento/consumo", (req, res) => {
  const { alimento_id, lote_nombre, cantidad_animales, kg_por_animal_dia, fecha_inicio, fecha_fin, notas } = req.body;
  db.prepare("INSERT INTO consumos_programados (alimento_id,lote_nombre,cantidad_animales,kg_por_animal_dia,fecha_inicio,fecha_fin,notas) VALUES (?,?,?,?,?,?,?)")
    .run(alimento_id, lote_nombre||'', cantidad_animales||1, kg_por_animal_dia, fecha_inicio, fecha_fin||null, notas||'');
  res.json({ mensaje: `✅ Consumo programado: ${cantidad_animales} animales x ${kg_por_animal_dia}kg/día` });
});

app.post("/api/stock/alimento/descontar", (req, res) => {
  // Descontar consumo diario de todos los programados activos
  const hoy = new Date().toISOString().slice(0,10);
  const consumos = db.prepare("SELECT * FROM consumos_programados WHERE activo = 1 AND fecha_inicio <= ? AND (fecha_fin IS NULL OR fecha_fin >= ?)").all(hoy, hoy);
  ...
  for (const c of consumos) {
    ...
    db.prepare("UPDATE stock_alimento SET stock_actual = MAX(0, stock_actual - ?) WHERE id = ?").run(consumoDia, c.alimento_id);
    db.prepare("INSERT INTO alimento_movimientos (alimento_id,fecha,tipo,cantidad,detalle) VALUES (?,?,'CONSUMO',?,?)")
      .run(c.alimento_id, hoy, consumoDia, `${c.lote_nombre}: ${c.cantidad_animales}x${c.kg_por_animal_dia}kg`);
    totalDescontado += consumoDia;
  }
  res.json({ mensaje: `✅ Descontados ${totalDescontado.toFixed(1)}kg de ${consumos.length} consumos activos` });
});
...
app.get("/api/productividad", (req, res) => {
  // GDP promedio por padre
  const gdpPorPadre = db.prepare(`
    SELECT a.padre_rp as padre, COUNT(*) as n_hijos,
      AVG(p.gdp) as gdp_prom,
      AVG(CASE WHEN p.contexto='DESTETE' THEN p.peso END) as peso_dest_prom,
      AVG(CASE WHEN p.contexto='NACIMIENTO' THEN p.peso END) as peso_nac_prom
    FROM animales a 
    JOIN pesadas p ON p.animal_id = a.id
    WHERE a.padre_rp IS NOT NULL AND a.padre_rp != ''
    GROUP BY a.padre_rp HAVING n_hijos >= 2
...
app.get("/api/alertas", (req, res) => {
  ...
  // 4. Stock bajo de insumos
  const stockBajo = db.prepare("SELECT * FROM stock_insumos WHERE stock_actual <= stock_minimo AND stock_minimo > 0").all();
  stockBajo.forEach(s => alertas.push({ tipo: 'STOCK', prioridad: 'ALTA', mensaje: `${s.nombre}: stock ${s.stock_actual} ${s.unidad} (mínimo: ${s.stock_minimo})` }));
  
  // 5. Alimento por acabarse (<7 días)
  const alimentos = db.prepare("SELECT * FROM stock_alimento").all();
  const consumos = db.prepare("SELECT * FROM consumos_programados WHERE activo = 1").all();
  alimentos.forEach(a => {
    const csDia = consumos.filter(c => c.alimento_id === a.id).reduce((s, c) => s + c.cantidad_animales * c.kg_por_animal_dia, 0);
    if (csDia > 0) {
      const dias = Math.floor(a.stock_actual / csDia);
      if (dias < 7) alertas.push({ tipo: 'ALIMENTO', prioridad: dias < 3 ? 'URGENTE' : 'ALTA', mensaje: `${a.nombre}: quedan ${dias} días (${a.stock_actual.toFixed(0)}kg, consumo ${csDia.toFixed(0)}kg/día)` });
    }
  });
  
  // 6. Insumos vencidos o por vencer
  const porVencer = db.prepare("SELECT * FROM stock_insumos WHERE vencimiento != '' AND vencimiento <= ? AND stock_actual > 0").all(
    new Date(Date.now() + 30*24*60*60*1000).toISOString().slice(0,10)
  );
  porVencer.forEach(s => alertas.push({ tipo: 'VENCIMIENTO', prioridad: s.vencimiento <= hoy ? 'URGENTE' : 'MEDIA', mensaje: `${s.nombre} vence ${s.vencimiento} (${s.stock_actual} ${s.unidad})` }));
...
// ══════════════════════════════════════════════════════════════════════════════
// ── TRACKING DE ENVÍOS A EVALUACIÓN (INIA / BREEDPLAN) ────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
const TIPOS_ENVIO = ['NACIMIENTO','DESTETE','18MESES','SERVICIO'];
const ORGANISMOS_ENVIO = ['INIA','BREEDPLAN'];

function resolverAnimalesPorRps(rps) {
  const encontrados = [], noEnc = [];
  for (const rp of (rps || [])) {
    const clave = String(rp).trim();
    if (!clave) continue;
    let a = buscarAnimal(clave);
    if (!a) a = buscarAnimalTodos(clave);
    if (a) encontrados.push(a); else noEnc.push(clave);
  }
  return { encontrados, noEnc };
}

function logEnvioEvento(envioId, animalId, estado, detalle, fecha) {
  db.prepare("INSERT INTO envio_eventos (envio_id, animal_id, fecha, estado, detalle) VALUES (?, ?, ?, ?, ?)")
    .run(envioId, animalId || null, fecha || new Date().toISOString().slice(0,10), estado || null, detalle || null);
}

// LISTA de envíos (con filtros opcionales y conteos por estado)
app.get("/api/envios", (req, res) => {
  try {
    const { organismo, tipo, temporada } = req.query;
    const cond = [], params = [];
    if (organismo) { cond.push("organismo = ?"); params.push(organismo); }
    if (tipo) { cond.push("tipo = ?"); params.push(tipo); }
    if (temporada) { cond.push("temporada = ?"); params.push(temporada); }
    const where = cond.length ? "WHERE " + cond.join(" AND ") : "";
    const envios = db.prepare(`SELECT * FROM envios_evaluacion ${where} ORDER BY temporada DESC, fecha_envio DESC, created_at DESC`).all(...params);
    const out = envios.map(e => {
      const n = db.prepare("SELECT COUNT(*) n FROM envio_animales WHERE envio_id = ?").get(e.id).n;
      const obs = db.prepare("SELECT COUNT(*) n FROM envio_animales WHERE envio_id = ? AND estado IN ('OBSERVADO','RECHAZADO')").get(e.id).n;
      const corr = db.prepare("SELECT COUNT(*) n FROM envio_animales WHERE envio_id = ? AND estado = 'CORREGIDO'").get(e.id).n;
      return { ...e, n_animales: n, n_observados: obs, n_corregidos: corr };
    });
    res.json(out);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// MATRIZ para dashboard: agrupa por temporada → organismo → tipo
app.get("/api/envios/matriz", (req, res) => {
  try {
    const envios = db.prepare("SELECT * FROM envios_evaluacion ORDER BY temporada DESC, organismo, tipo").all();
    const rows = envios.map(e => {
      const n = db.prepare("SELECT COUNT(*) n FROM envio_animales WHERE envio_id = ?").get(e.id).n;
      const obs = db.prepare("SELECT COUNT(*) n FROM envio_animales WHERE envio_id = ? AND estado IN ('OBSERVADO','RECHAZADO')").get(e.id).n;
      return { ...e, n_animales: n, n_observados: obs };
    });
    const totales = {
      envios: rows.length,
      animales: rows.reduce((s, r) => s + r.n_animales, 0),
      observados: rows.reduce((s, r) => s + r.n_observados, 0),
      por_organismo: {}
    };
    for (const o of ORGANISMOS_ENVIO) {
      totales.por_organismo[o] = rows.filter(r => r.organismo === o).reduce((s, r) => s + r.n_animales, 0);
    }
    res.json({ envios: rows, totales, tipos: TIPOS_ENVIO, organismos: ORGANISMOS_ENVIO });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// DETALLE de un envío (cabecera + animales + bitácora)
app.get("/api/envios/:id", (req, res) => {
  try {
    const e = db.prepare("SELECT * FROM envios_evaluacion WHERE id = ?").get(parseInt(req.params.id));
    if (!e) return res.status(404).json({ error: "Envío no encontrado" });
    const animales = db.prepare(`
      SELECT ea.id AS ea_id, ea.estado, ea.observacion, a.id AS animal_id, a.rp, a.chip, a.categoria, a.sexo, a.registro
      FROM envio_animales ea JOIN animales a ON a.id = ea.animal_id
      WHERE ea.envio_id = ? ORDER BY a.rp
    `).all(e.id);
    const eventos = db.prepare(`
      SELECT ev.*, a.rp FROM envio_eventos ev LEFT JOIN animales a ON a.id = ev.animal_id
      WHERE ev.envio_id = ? ORDER BY ev.fecha DESC, ev.created_at DESC
    `).all(e.id);
    res.json({ ...e, animales, eventos });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// CREAR envío (resuelve rps → animales, crea cabecera + detalle + evento inicial)
app.post("/api/envios", (req, res) => {
  try {
    const { organismo, tipo, temporada, fecha_envio, archivo, notas, estado, rps } = req.body;
    if (!ORGANISMOS_ENVIO.includes(organismo)) return res.status(400).json({ error: "Organismo inválido (INIA | BREEDPLAN)" });
    if (!TIPOS_ENVIO.includes(tipo)) return res.status(400).json({ error: "Tipo inválido (NACIMIENTO | DESTETE | 18MESES | SERVICIO)" });
    const { encontrados, noEnc } = resolverAnimalesPorRps(rps);
    if (!encontrados.length) return res.status(404).json({ error: `Ningún animal encontrado. No encontrados: ${noEnc.join(', ')}` });
    const fecha = fecha_envio || new Date().toISOString().slice(0,10);
    const est = estado || 'ENVIADO';
    const tx = db.transaction(() => {
      const r = db.prepare(`INSERT INTO envios_evaluacion (organismo, tipo, temporada, fecha_envio, archivo, estado, notas) VALUES (?, ?, ?, ?, ?, ?, ?)`)
        .run(organismo, tipo, temporada || null, fecha, archivo || null, est, notas || null);
      const envioId = r.lastInsertRowid;
      const ins = db.prepare("INSERT OR IGNORE INTO envio_animales (envio_id, animal_id, estado) VALUES (?, ?, ?)");
      for (const a of encontrados) ins.run(envioId, a.id, est);
      logEnvioEvento(envioId, null, est, `Envío creado con ${encontrados.length} animales`, fecha);
      return envioId;
    });
    const envioId = tx();
    res.json({ mensaje: `✅ Envío ${organismo} ${tipo} ${temporada || ''} creado con ${encontrados.length} animales.${noEnc.length ? ` No encontrados: ${noEnc.join(', ')}` : ''}`, id: envioId, encontrados: encontrados.length, no_encontrados: noEnc });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ACTUALIZAR cabecera de envío (estado / notas / fecha / archivo) — registra en bitácora si cambia el estado
app.put("/api/envios/:id", (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const e = db.prepare("SELECT * FROM envios_evaluacion WHERE id = ?").get(id);
    if (!e) return res.status(404).json({ error: "Envío no encontrado" });
    const { estado, notas, fecha_envio, archivo, temporada } = req.body;
    db.prepare(`UPDATE envios_evaluacion SET estado=?, notas=?, fecha_envio=?, archivo=?, temporada=? WHERE id=?`)
      .run(estado || e.estado, notas !== undefined ? notas : e.notas, fecha_envio || e.fecha_envio,
           archivo !== undefined ? archivo : e.archivo, temporada !== undefined ? temporada : e.temporada, id);
    if (estado && estado !== e.estado) logEnvioEvento(id, null, estado, `Estado del envío: ${e.estado} → ${estado}`, req.body.fecha);
    res.json({ mensaje: `✅ Envío actualizado.` });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ELIMINAR envío (cabecera + detalle + bitácora)
app.delete("/api/envios/:id", (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const tx = db.transaction(() => {
      db.prepare("DELETE FROM envio_eventos WHERE envio_id = ?").run(id);
      db.prepare("DELETE FROM envio_animales WHERE envio_id = ?").run(id);
      db.prepare("DELETE FROM envios_evaluacion WHERE id = ?").run(id);
    });
    tx();
    res.json({ mensaje: "🗑️ Envío eliminado." });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// AGREGAR animales a un envío existente
app.post("/api/envios/:id/animales", (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const e = db.prepare("SELECT * FROM envios_evaluacion WHERE id = ?").get(id);
    if (!e) return res.status(404).json({ error: "Envío no encontrado" });
    const { encontrados, noEnc } = resolverAnimalesPorRps(req.body.rps);
    const ins = db.prepare("INSERT OR IGNORE INTO envio_animales (envio_id, animal_id, estado) VALUES (?, ?, 'ENVIADO')");
    let agregados = 0;
    const tx = db.transaction(() => { for (const a of encontrados) { const r = ins.run(id, a.id); if (r.changes) agregados++; } });
    tx();
    if (agregados) logEnvioEvento(id, null, null, `Se agregaron ${agregados} animales`, req.body.fecha);
    res.json({ mensaje: `✅ ${agregados} animales agregados.${noEnc.length ? ` No encontrados: ${noEnc.join(', ')}` : ''}`, agregados, no_encontrados: noEnc });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// QUITAR un animal del envío
app.delete("/api/envios/:id/animales/:animalId", (req, res) => {
  try {
    const id = parseInt(req.params.id), aid = parseInt(req.params.animalId);
    const r = db.prepare("DELETE FROM envio_animales WHERE envio_id = ? AND animal_id = ?").run(id, aid);
    res.json({ mensaje: r.changes ? "✅ Animal quitado del envío." : "No estaba en el envío." });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// ACTUALIZAR estado por animal (corrección / observación) — queda en la bitácora
app.put("/api/envios/:id/animales/:animalId", (req, res) => {
  try {
    const id = parseInt(req.params.id), aid = parseInt(req.params.animalId);
    const ea = db.prepare("SELECT * FROM envio_animales WHERE envio_id = ? AND animal_id = ?").get(id, aid);
    if (!ea) return res.status(404).json({ error: "Animal no está en este envío" });
    const { estado, observacion, fecha } = req.body;
    db.prepare("UPDATE envio_animales SET estado = ?, observacion = ? WHERE id = ?")
      .run(estado || ea.estado, observacion !== undefined ? observacion : ea.observacion, ea.id);
    if (estado && estado !== ea.estado) {
      const an = db.prepare("SELECT rp FROM animales WHERE id = ?").get(aid);
      logEnvioEvento(id, aid, estado, `${an ? 'RP ' + an.rp + ': ' : ''}${ea.estado} → ${estado}${observacion ? ' (' + observacion + ')' : ''}`, fecha);
    }
    res.json({ mensaje: "✅ Estado del animal actualizado." });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// EVENTO manual de seguimiento / corrección
app.post("/api/envios/:id/evento", (req, res) => {
  try {
    const id = parseInt(req.params.id);
    const e = db.prepare("SELECT * FROM envios_evaluacion WHERE id = ?").get(id);
    if (!e) return res.status(404).json({ error: "Envío no encontrado" });
    const { animal_id, estado, detalle, fecha } = req.body;
    logEnvioEvento(id, animal_id || null, estado || null, detalle || null, fecha);
    res.json({ mensaje: "✅ Evento registrado." });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get("/", (req, res) => {
  ...
  res.json({ status: "GANADERÍA Bot activo 🐂", version: "5.0-multicampo", campos });
});
...
app.listen(PORT, () => {
  ...
  // ── CRON: Descuento diario de alimento + alertas WhatsApp ──
  // Se ejecuta todos los días a las 8:00 AM Uruguay (UTC-3 = 11:00 UTC)
  function programarDescuentoDiario() {
    const ahora = new Date();
    // Calcular próxima ejecución a las 11:00 UTC (8:00 UY)
    let prox = new Date(ahora);
    prox.setUTCHours(11, 0, 0, 0);
    if (prox <= ahora) prox.setDate(prox.getDate() + 1);
    const msHastaProx = prox - ahora;
    
    console.log(`[CRON] Próximo descuento de alimento: ${prox.toISOString()} (en ${(msHastaProx/3600000).toFixed(1)}h)`);
    
    setTimeout(() => {
      ejecutarDescuentoDiario();
      // Reprogramar para mañana
      setInterval(ejecutarDescuentoDiario, 24 * 60 * 60 * 1000);
    }, msHastaProx);
  }
  
  async function ejecutarDescuentoDiario() {
    const hoy = new Date().toISOString().slice(0, 10);
    console.log(`[CRON] Ejecutando descuento diario: ${hoy}`);
    
    // Recorrer cada campo
    for (const [campoKey, campo] of Object.entries(CAMPOS)) {
      const campoDb = databases[campoKey];
      if (!campoDb) continue;
      
      try {
        // Verificar si ya se descontó hoy
        const yaDesconto = campoDb.prepare("SELECT COUNT(*) as n FROM alimento_movimientos WHERE fecha = ? AND tipo = 'CONSUMO_AUTO'").get(hoy);
        if (yaDesconto.n > 0) { console.log(`[CRON] ${campoKey}: ya descontado hoy`); continue; }
        
        const consumos = campoDb.prepare("SELECT * FROM consumos_programados WHERE activo = 1 AND fecha_inicio <= ? AND (fecha_fin IS NULL OR fecha_fin >= ?)").all(hoy, hoy);
        let totalDescontado = 0;
        
        for (const c of consumos) {
          const consumoDia = c.cantidad_animales * c.kg_por_animal_dia;
          campoDb.prepare("UPDATE stock_alimento SET stock_actual = MAX(0, stock_actual - ?) WHERE id = ?").run(consumoDia, c.alimento_id);
          campoDb.prepare("INSERT INTO alimento_movimientos (alimento_id, fecha, tipo, cantidad, detalle) VALUES (?, ?, 'CONSUMO_AUTO', ?, ?)")
            .run(c.alimento_id, hoy, consumoDia, `Auto: ${c.lote_nombre} ${c.cantidad_animales}×${c.kg_por_animal_dia}kg`);
          totalDescontado += consumoDia;
        }
        
        if (totalDescontado > 0) console.log(`[CRON] ${campoKey}: descontados ${totalDescontado.toFixed(1)}kg`);
        
        // Verificar alertas de stock bajo y enviar WhatsApp
        if (campo.whatsapp && campo.admin && twilioClient) {
          const alimentos = campoDb.prepare("SELECT * FROM stock_alimento").all();
          const consumosActivos = campoDb.prepare("SELECT * FROM consumos_programados WHERE activo = 1").all();
          let alertaMsg = '';
          
          alimentos.forEach(a => {
            const csDia = consumosActivos.filter(c => c.alimento_id === a.id).reduce((s, c) => s + c.cantidad_animales * c.kg_por_animal_dia, 0);
            if (csDia > 0) {
              const diasRestantes = Math.floor(a.stock_actual / csDia);
              if (diasRestantes <= 10) {
                alertaMsg += `${diasRestantes <= 3 ? '🚨' : '⚠️'} ${a.nombre}: ${diasRestantes} días (${a.stock_actual.toFixed(0)}kg, consumo ${csDia.toFixed(0)}kg/día)\n`;
              }
            }
          });
          
          // Insumos bajo mínimo
          const stockBajo = campoDb.prepare("SELECT * FROM stock_insumos WHERE stock_actual <= stock_minimo AND stock_minimo > 0").all();
          stockBajo.forEach(s => {
            alertaMsg += `⚠️ ${s.nombre}: ${s.stock_actual} ${s.unidad} (mínimo: ${s.stock_minimo})\n`;
          });
          
          if (alertaMsg) {
            const campoTwilio = getTwilioClient(campoKey);
            if (campoTwilio) {
              const msgCompleto = `🔔 *ALERTA STOCK — ${campo.nombre}*\n📅 ${hoy}\n\n${alertaMsg}\n📦 Consumo diario total: ${totalDescontado.toFixed(0)}kg`;
              try {
                await campoTwilio.messages.create({
                  body: msgCompleto,
                  from: campo.whatsapp,
                  to: campo.admin
                });
                console.log(`[CRON] WhatsApp enviado a ${campo.nombre}: alerta stock`);
              } catch(e) {
                console.error(`[CRON] Error WhatsApp ${campoKey}:`, e.message);
              }
            }
          }
        }
        ...
        console.error(`[CRON] Error ${campoKey}:`, e.message);
      }
    }
  }
  
  programarDescuentoDiario();
});
