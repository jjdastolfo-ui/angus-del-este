...
Stock insumos: ${JSON.stringify(db.prepare("SELECT nombre,stock_actual,unidad FROM stock_insumos WHERE stock_actual>0").all())}
Stock alimento: ${JSON.stringify(db.prepare("SELECT nombre,stock_actual FROM stock_alimento").all())}
Stock genética: ${JSON.stringify(db.prepare("SELECT tipo,toro_nombre,donante_nombre,cantidad FROM stock_genetica WHERE cantidad>0").all())}`;
}
