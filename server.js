...
// ── HEALTH ────────────────────────────────────────────────────────────────────
app.get("/", (req, res) => res.json({ status: "ANGUS DEL ESTE Bot activo 🐂", version: "4.1" }));
...
*/
app.get("/api/informe-inia/destete", async (req, res) => {
  if (!ExcelJS) return res.status(500).json({ error: "exceljs no instalado" });
  
  try {
    const generacion = req.query.generacion || `Primavera - ${new Date().getFullYear()}`;
    const manejo = parseInt(req.query.manejo) || 1;
    const fechaPesada = req.query.fecha_pesada || null;
    const cabana = "ANGUS DEL ESTE - 39100";
    const raza = "ABERDEEN ANGUS";
    
    // Buscar terneros para el informe: nacidos en el año de la generación
    // Extraer año de la generación (ej: "Primavera - 2025" → 2025)
    const anioMatch = generacion.match(/(\d{4})/);
    const anioNac = anioMatch ? anioMatch[1] : new Date().getFullYear().toString();
    
    const terneros = db.prepare(`
      SELECT a.*, 
        (SELECT peso FROM pesadas WHERE animal_id = a.id AND contexto = 'NACIMIENTO' ORDER BY fecha DESC LIMIT 1) as peso_nac,
        (SELECT peso FROM pesadas WHERE animal_id = a.id AND contexto = 'DESTETE' ORDER BY fecha DESC LIMIT 1) as peso_destete
      FROM animales a 
      WHERE a.fecha_nac LIKE ? 
      AND a.estado = 'ACTIVO'
      ORDER BY a.registro DESC, a.rp ASC
    `).all(`${anioNac}-%`);
    
    if (!terneros.length) return res.status(404).json({ error: `No hay terneros nacidos en ${anioNac}` });
    ...
    ws.getCell('AA4').value = generacion;
    ...
    for (const t of terneros) {
      ...
      // Buscar HBU del ternero desde notas (si tiene HBU:XXXXX)
      let hbu = '';
      if (t.notas) {
        const hbuMatch = t.notas.match(/HBU:(\w+)/);
        if (hbuMatch) hbu = hbuMatch[1];
      }
      
      // Buscar HBU de la madre
      let madreHbu = '';
      if (infoMadre.rp) {
        const madreAnimal = buscarAnimal(infoMadre.rp);
        if (madreAnimal && madreAnimal.notas) {
          const m = madreAnimal.notas.match(/HBU:(\w+)/);
          if (m) madreHbu = m[1];
        }
        if (!madreHbu && madreAnimal && madreAnimal.madre_hba) madreHbu = madreAnimal.madre_hba;
      }
      
      // Buscar HBU del padre
      let padreHbu = '';
      if (t.padre_rp) {
        const padreAnimal = buscarAnimal(t.padre_rp);
        if (padreAnimal && padreAnimal.notas) {
          const m = padreAnimal.notas.match(/HBU:(\w+)/);
          if (m) padreHbu = m[1];
        }
        if (!padreHbu && padreAnimal && padreAnimal.padre_hba) padreHbu = padreAnimal.padre_hba;
      }
    ...
    // Enviar
    const filename = `INIA_Destete_${cabana.replace(/\s/g,'_')}_${anioNac}.xlsx`;
...
*/
app.post("/api/informe-inia/completar-destete", express.raw({ type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', limit: '10mb' }), async (req, res) => {
  if (!ExcelJS) return res.status(500).json({ error: "exceljs no instalado" });
  ...
  try {
    ...
    // Recorrer desde fila 13 hasta el final de datos
    ws.eachRow((row, rowNum) => {
      if (rowNum < 13) return;
      
      const rp = row.getCell(12).value; // Col L = RP del ternero
      if (!rp) return;
      ...
      if (!animal) {
        noEncontrados.push(rpStr);
        return;
      }
      
      // Peso destete (col U = 21)
      const pesoDestete = getPesoDestete(animal.id);
      if (pesoDestete && !row.getCell(21).value) {
        row.getCell(21).value = pesoDestete;
      }
      
      // Manejo (col X = 24)
      if (!row.getCell(24).value) {
        row.getCell(24).value = manejo;
      }
      
      // Lote (col Y = 25)
      const lote = getLoteAnimal(animal.id);
      if (lote && !row.getCell(25).value) {
        row.getCell(25).value = lote;
      }
      
      // Padre RP (col AC = 29) — solo si no tiene
      if (!row.getCell(29).value && animal.padre_rp) {
        row.getCell(29).value = animal.padre_rp;
      }
      ...
      if (animal.madre_rp) {
        ...
        // Madre RP (col AF = 32) — solo si no tiene
        if (!row.getCell(32).value) {
          row.getCell(32).value = animal.madre_rp;
        }
        
        // Madre Kg destete (col AG = 33)
        if (infoMadre.peso_adulta && !row.getCell(33).value) {
          row.getCell(33).value = infoMadre.peso_adulta;
        }
        
        // Madre CC (col AH = 34)
        if (infoMadre.cc && !row.getCell(34).value) {
          row.getCell(34).value = infoMadre.cc;
        }
      }
    ...
    // Devolver el archivo completado
    const filename = `299_39100_ANGUS_DEL_ESTE_completo.xlsx`;
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.setHeader('X-Completados', completados);
    res.setHeader('X-No-Encontrados', noEncontrados.join(','));
    await wb.xlsx.write(res);
...
/*
  GET /api/informe-inia/18meses?generacion=Primavera%20-%202024&manejo=1
  
  Genera Excel formato INIA para toros y vaquillonas de 18 meses.
  Campos: RP, HBU, Sexo, Fecha Nac, Peso 18m, CE (machos), Manejo, Lote, Padre, Madre
*/
app.get("/api/informe-inia/18meses", async (req, res) => {
  if (!ExcelJS) return res.status(500).json({ error: "exceljs no instalado" });
  
  try {
    const generacion = req.query.generacion || `Primavera - ${new Date().getFullYear() - 1}`;
    const manejo = parseInt(req.query.manejo) || 1;
    const cabana = "ANGUS DEL ESTE - 39100";
    
    const anioMatch = generacion.match(/(\d{4})/);
    const anioNac = anioMatch ? anioMatch[1] : String(new Date().getFullYear() - 1);
    
    // Animales nacidos en ese año que tengan ~18 meses (recría/toro/vaquillona)
    const animales = db.prepare(`
      SELECT a.*,
        (SELECT peso FROM pesadas WHERE animal_id = a.id AND contexto = '18MESES' ORDER BY fecha DESC LIMIT 1) as peso_18m,
        (SELECT peso FROM pesadas WHERE animal_id = a.id AND contexto = 'AÑO' ORDER BY fecha DESC LIMIT 1) as peso_ano,
        (SELECT valor FROM mediciones WHERE animal_id = a.id AND tipo = 'CE' ORDER BY fecha DESC LIMIT 1) as ce,
        (SELECT valor FROM mediciones WHERE animal_id = a.id AND tipo = 'ALTURA' ORDER BY fecha DESC LIMIT 1) as altura,
        (SELECT valor FROM mediciones WHERE animal_id = a.id AND tipo = 'FRAME' ORDER BY fecha DESC LIMIT 1) as frame
      FROM animales a 
      WHERE a.fecha_nac LIKE ? AND a.estado = 'ACTIVO'
      ORDER BY a.registro DESC, a.sexo, a.rp
    `).all(`${anioNac}-%`);
    
    if (!animales.length) return res.status(404).json({ error: `No hay animales nacidos en ${anioNac}` });
    ...
    // Encabezado
    ws.mergeCells('A1:L1');
    ws.getCell('A1').value = `ANGUS DEL ESTE — Informe 18 Meses — ${generacion}`;
    ws.getCell('A1').font = { bold: true, size: 14 };
    ws.getCell('A2').value = `Cabaña: ${cabana} | Raza: ABERDEEN ANGUS | Generación: ${generacion}`;
    ws.getCell('A2').font = { size: 10, color: { argb: 'FF666666' } };
    
    // Headers fila 4
    const hdrs = ['Reg.','RP','HBU','Sexo','Fecha Nac','Peso 18m','CE','Altura','Frame','Manejo','Lote','Padre RP','Madre RP'];
    hdrs.forEach((h, i) => {
    ...
    for (const a of animales) {
      ...
      let hbu = '';
      if (a.notas) { const m = a.notas.match(/HBU:(\w+)/); if (m) hbu = m[1]; }
      const lote = getLoteAnimal(a.id) || '';
      
      ws.getCell(row, 1).value = reg;
      ws.getCell(row, 2).value = a.rp;
      ws.getCell(row, 3).value = hbu;
      ...
      if (a.peso_18m) ws.getCell(row, 6).value = a.peso_18m;
      else if (a.peso_ano) ws.getCell(row, 6).value = a.peso_ano;
      if (a.ce && a.sexo === 'MACHO') ws.getCell(row, 7).value = a.ce;
      if (a.altura) ws.getCell(row, 8).value = a.altura;
      if (a.frame) ws.getCell(row, 9).value = a.frame;
      ws.getCell(row, 10).value = manejo;
      if (lote) ws.getCell(row, 11).value = lote;
      if (a.padre_rp) ws.getCell(row, 12).value = a.padre_rp;
      if (a.madre_rp) ws.getCell(row, 13).value = a.madre_rp;
      row++;
    }
    
    // Anchos
    [6,8,8,6,12,10,6,8,6,8,8,12,12].forEach((w, i) => ws.getColumn(i+1).width = w);
    ...
    const filename = `INIA_18Meses_ADE_${anioNac}.xlsx`;
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    await wb.xlsx.write(res);
...
*/
app.get("/api/informe-inia/servicio", async (req, res) => {
  if (!ExcelJS) return res.status(500).json({ error: "exceljs no instalado" });
  
  try {
    const temporada = req.query.temporada || String(new Date().getFullYear());
    const cabana = "ANGUS DEL ESTE - 39100";
    ...
    const servicios = db.prepare(`
      ...
      JOIN animales a ON a.id = s.animal_id 
      WHERE s.temporada = ?
      ORDER BY a.registro DESC, a.rp
    `).all(temporada);
    
    if (!servicios.length) return res.status(404).json({ error: `No hay servicios en temporada ${temporada}` });
    ...
    // Encabezado
    ws.mergeCells('A1:N1');
    ws.getCell('A1').value = `ANGUS DEL ESTE — Informe Servicio — Temporada ${temporada}`;
    ws.getCell('A1').font = { bold: true, size: 14 };
    ws.getCell('A2').value = `Cabaña: ${cabana} | Raza: ABERDEEN ANGUS`;
    ws.getCell('A2').font = { size: 10, color: { argb: 'FF666666' } };
