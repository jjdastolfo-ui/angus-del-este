...
app.get("/api/repro/toros-plantel", (req, res) => {
  ...
  try {
    res.json(base.prepare(`
      SELECT id, rp, raza, hbu, registro, fecha_nac
      FROM animales
      WHERE sexo='M' AND estado='ACTIVO'
        AND (upper(COALESCE(categoria,'')) LIKE '%TORO%' OR upper(COALESCE(destino,'')) LIKE '%PLANTEL%')
      ORDER BY rp
    `).all());
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get("/api/repro/vacas-disponibles", (req, res) => {
  ...
  try {
    res.json(base.prepare(`
      ...
      FROM animales a
      WHERE a.sexo='H' AND a.estado='ACTIVO'
        AND a.id NOT IN (
