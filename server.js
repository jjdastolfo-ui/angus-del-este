...
try { importador = require("./importador.js"); } catch(e) { console.log("importador.js no disponible:", e.message); }
let pdfjsLib;
...
app.post("/webhook-interno", async (req, res) => {
  try {
    ...
    // ── INTERCEPT: ARCHIVO ADJUNTO (parser viejo, respaldo) ──
    // Sólo corre si el importador inteligente no está cargado.
    if (fileData && !importador) {
      try {
        const buf = Buffer.from(fileData, 'base64');
        let datos = [];
