...
{"accion":"registrar_sanidad","rp":"","tipo":"VACUNA/TRATAMIENTO/DESPARASITACION","producto":"","fecha":""}
...
STOCK:
{"accion":"crear_alimento","nombre":"","tipo":"RACION","stock_actual":0,"costo_por_kg":0}
{"accion":"compra_alimento","nombre":"","cantidad":0,"costo_total":0}
{"accion":"ajustar_stock","nombre":"","stock_correcto":0}
{"accion":"borrar_movimiento_alimento","nombre":""}
{"accion":"programar_consumo","nombre":"","lote":"","cantidad_animales":0,"kg_por_animal_dia":0}
{"accion":"eliminar_consumo","lote":""}
...
REGLAS: ADE es prefijo (ADE2=2). Fecha YYYY-MM-DD, sin fecha→HOY. Tacto: PREÑADA_IATF/PREÑADA_TORO/VACIA. Se vincula al último servicio sin resultado. Parto: ±15d IATF+282=IATF, sino repaso. Solo hembras 12+m en servicio. Varios animales→1 JSON con array. Pregunta→consulta. Sin entender→texto preguntar.

DATOS:
Rodeo: ${JSON.stringify(resumen)}
Lotes: ${JSON.stringify(lotesResumen)}
Alimentos: ${JSON.stringify((() => { try { return db.prepare("SELECT nombre,stock_actual FROM stock_alimento").all(); } catch(e) { return []; } })())}`;
...
stock_alimento(id, nombre, stock_actual, costo_por_kg)
...
function ejecutarAccion(accion) {
  ...
  if (accion.accion === "compra_alimento") {
    ...
    let al = db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) = LOWER(?)").get(nombre);
    if (!al) al = db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${nombre}%`);
    if (!al && nombre) { const p=nombre.split(/\s+/)[0]; al=db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${p}%`); }
    if (!al) {
      const todos = db.prepare("SELECT nombre FROM stock_alimento").all();
      return `❌ No encontré alimento "${nombre}". Existentes: ${todos.map(t=>t.nombre).join(', ')||'ninguno'}`;
    }
    ...
    db.prepare("UPDATE stock_alimento SET stock_actual = ROUND(stock_actual + ?, 2) WHERE id = ?").run(cantidad, al.id);
    if (costo_total && cantidad) db.prepare("UPDATE stock_alimento SET costo_por_kg = ROUND(?, 4) WHERE id = ?").run(costo_total/cantidad, al.id);
    const act = db.prepare("SELECT stock_actual FROM stock_alimento WHERE id = ?").get(al.id);
    return `✅ Compra: ${cantidad}kg de ${al.nombre}\n💰 $${costo_total||0}\n📦 Stock: ${Math.round(act.stock_actual)}kg`;
  }

  // Ajustar stock manualmente (corregir errores)
  if (accion.accion === "ajustar_stock") {
    const { nombre, stock_correcto } = accion;
    let al = db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${nombre}%`);
    if (!al && nombre) { const p=nombre.split(/\s+/)[0]; al=db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${p}%`); }
    if (!al) return `❌ No encontré alimento "${nombre}".`;
    const anterior = al.stock_actual;
    db.prepare("UPDATE stock_alimento SET stock_actual = ? WHERE id = ?").run(stock_correcto, al.id);
    db.prepare("INSERT INTO alimento_movimientos (alimento_id,fecha,tipo,cantidad,costo_total,detalle) VALUES (?,?,'AJUSTE',?,0,?)")
      .run(al.id, hoy, stock_correcto - anterior, `Ajuste: ${Math.round(anterior)}→${stock_correcto}kg`);
    return `✅ Stock ${al.nombre} ajustado: ${Math.round(anterior)}kg → ${stock_correcto}kg`;
  }

  // Borrar último movimiento de alimento
  if (accion.accion === "borrar_movimiento_alimento") {
    const { nombre } = accion;
    let al = db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${nombre}%`);
    if (!al && nombre) { const p=nombre.split(/\s+/)[0]; al=db.prepare("SELECT * FROM stock_alimento WHERE LOWER(nombre) LIKE LOWER(?)").get(`%${p}%`); }
    if (!al) return `❌ No encontré alimento "${nombre}".`;
    const ult = db.prepare("SELECT * FROM alimento_movimientos WHERE alimento_id = ? ORDER BY id DESC LIMIT 1").get(al.id);
    if (!ult) return `❌ No hay movimientos para ${al.nombre}.`;
    // Revertir
    if (ult.tipo === 'COMPRA') {
      db.prepare("UPDATE stock_alimento SET stock_actual = ROUND(stock_actual - ?, 2) WHERE id = ?").run(ult.cantidad, al.id);
    } else if (ult.tipo === 'CONSUMO') {
      db.prepare("UPDATE stock_alimento SET stock_actual = ROUND(stock_actual + ?, 2) WHERE id = ?").run(Math.abs(ult.cantidad), al.id);
    }
    db.prepare("DELETE FROM alimento_movimientos WHERE id = ?").run(ult.id);
    const act = db.prepare("SELECT stock_actual FROM stock_alimento WHERE id = ?").get(al.id);
    return `🗑️ Último movimiento eliminado: ${ult.tipo} ${ult.cantidad}kg\n📦 Stock actual: ${Math.round(act.stock_actual)}kg`;
  }
...
// Matriz por pedigree (PP / SA / GENERAL). Categorías: Toros · Vacas · Vaquillonas
// · Terneros · Terneras. Cada una cuenta PLANTEL y VENTA por separado (destino).
//   → rodeo: [{ categoria, registro, plantel, venta }]
app.get("/api/rodeo-resumen", (req, res) => {
  const animales = db.prepare("SELECT id, categoria, registro, destino, sexo, fecha_nac FROM animales WHERE estado = 'ACTIVO'").all();
  const tactoStmt = db.prepare("SELECT resultado FROM servicios WHERE animal_id = ? AND resultado IS NOT NULL ORDER BY created_at DESC LIMIT 1");
  const grupos = {};
  const normReg = (r) => {
  ...
  const add = (categoria, registro, destino) => {
    ...
    if (destino === 'VENTA') grupos[k].venta++; else grupos[k].plantel++;
  };
  const hoy = new Date();
  const CORTE_TERNERO = 13; // meses: < 13m = ternero/a; 13m+ = adulto (toro / vaca / vaquillona)
  for (const a of animales) {
    const reg = a.registro;
    const dest = (a.destino || 'PLANTEL').toUpperCase();
    const catStored = (a.categoria || '').toUpperCase();
    const sexo = (a.sexo || '').toUpperCase();
    const edadMeses = a.fecha_nac ? Math.floor((hoy - new Date(a.fecha_nac)) / (1000 * 60 * 60 * 24 * 30.44)) : null;

    let cat;
    if (catStored === 'VACA') cat = 'Vacas';
    else if (catStored === 'TORO' || catStored === 'NOVILLO') cat = 'Toros';
    else if (catStored === 'VAQUILLONA') cat = 'Vaquillonas';
    else if (edadMeses !== null && edadMeses < CORTE_TERNERO) {
      cat = (sexo === 'HEMBRA') ? 'Terneras' : 'Terneros';
    } else {
      // adulto por edad (o sin fecha): macho → Toros; hembra → Vaca si preñada, si no Vaquillona
      if (sexo === 'MACHO') cat = 'Toros';
      else {
        const t = tactoStmt.get(a.id);
        cat = (t && t.resultado === 'PREÑADA') ? 'Vacas' : 'Vaquillonas';
      }
    }
    add(cat, reg, dest);
  }
  const ordenCat = { 'Toros': 1, 'Vacas': 2, 'Vaquillonas': 3, 'Terneros': 4, 'Terneras': 5 };
...
app.get("/api/integracion/improlux", (req, res) => {
  ...
  try {
    ...
    try {
      insumos = d.prepare(`
        SELECT nombre, tipo, unidad, stock_actual, costo_unitario, proveedor,
          ROUND(stock_actual * COALESCE(costo_unitario,0), 2) as valor_total
        FROM stock_insumos WHERE stock_actual > 0 ORDER BY tipo, nombre
      `).all();
    } catch(e) {}
    ...
    try {
      alimentos = d.prepare(`
        SELECT nombre, stock_actual, costo_por_kg,
          ROUND(stock_actual * COALESCE(costo_por_kg,0), 2) as valor_total
        FROM stock_alimento WHERE stock_actual > 0 ORDER BY nombre
      `).all();
    } catch(e) {}

    // Totales de valuación de insumos (lo que sí tiene costo cargado)
    const valorInsumos = insumos.reduce((s,i) => s + (i.valor_total||0), 0);
    const valorGenetica = genetica.reduce((s,g) => s + (g.valor_total||0), 0);
    const valorAlimentos = alimentos.reduce((s,a) => s + (a.valor_total||0), 0);
