  ...
  try {
    ...
    try {
      result = await anthropic.messages.create({
        ...
        max_tokens: 800,
        system: contexto,
        messages: historial.slice(-6), // solo últimos 6 mensajes para evitar timeout
      });
    } catch(apiErr) {
      console.error("API error, reintentando con historial limpio:", apiErr.message);
      // Retry sin historial
      try {
        result = await anthropic.messages.create({
          model: "claude-sonnet-4-6",
        ...
        console.error("API error retry falló:", apiErr2.message);
        return res.json({ respuesta: "❌ Error de conexión con el asistente. Intentá de nuevo." });
      }
    }
...
// Control manual de lote (con lista de RPs leídos)
app.post("/api/lotes/:id/control", (req, res) => {
  const loteId = parseInt(req.params.id);
  const { rps, fecha } = req.body;
  if (!Array.isArray(rps) || !rps.length) return res.status(400).json({ error: "Faltan RPs leídos" });
  ...
  const noEncontrados = [];
  for (const rp of rps) {
    const a = buscarAnimal(rp);
    if (a) leidosIds.push(a.id); else noEncontrados.push(rp);
  }
...
app.get("/api/stats/anuales", (req, res) => {
  ...
  const d = getDB(campo);
  try {
    const años = d.prepare(`
      SELECT temporada as anio, 
        COUNT(*) as servicios,
        SUM(CASE WHEN resultado IN ('PREÑADA_IATF','PREÑADA_TORO','PREÑADA') THEN 1 ELSE 0 END) as prenadas,
        SUM(CASE WHEN resultado = 'VACIA' THEN 1 ELSE 0 END) as vacias
      FROM servicios WHERE temporada IS NOT NULL
      GROUP BY temporada ORDER BY temporada
    `).all();
    const destetes = d.prepare(`
      SELECT substr(fecha,1,4) as anio, COUNT(*) as n, AVG(peso) as peso_prom
      FROM pesadas WHERE contexto = 'DESTETE' AND fecha IS NOT NULL
      GROUP BY substr(fecha,1,4) ORDER BY anio
    `).all();
    res.json({ años, destetes });
  } catch(e) { res.json({ años: [], destetes: [] }); }
});
