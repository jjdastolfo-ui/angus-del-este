...
app.get("/api/stats/vacas", (req, res) => {
  try {
    // PVA: peso promedio de la vaca en pesadas de contexto DESTETE (cuando se pesa la vaca al destete de sus crías)
    // Como no tenemos contexto "PVA" específico, usamos el último peso registrado como proxy
    // CC promedio: promedio de todas las mediciones de CC de cada vaca
    const vacas = db.prepare("SELECT id, rp FROM animales WHERE estado = 'ACTIVO' AND categoria = 'VACA'").all();
    const stats = {};
    
    for (const v of vacas) {
      // PVA: promedio de pesadas de la vaca en contexto ADULTA únicamente
      const pvaRow = db.prepare(`
        SELECT AVG(peso) as pva FROM pesadas 
        WHERE animal_id = ? AND contexto = 'ADULTA'
      `).get(v.id);
      
      // CC promedio de la vaca
      const ccRow = db.prepare(`
        SELECT AVG(valor) as cc_prom FROM mediciones 
        WHERE animal_id = ? AND tipo = 'CC' AND valor > 0
      `).get(v.id);
      
      // Peso promedio al destete de sus hijos (solo los que tienen peso de destete)
      const hijosDestete = db.prepare(`
        SELECT AVG(p.peso) as prom_dest, COUNT(*) as n FROM pesadas p
        JOIN animales a ON a.id = p.animal_id
        WHERE a.madre_rp = ? AND p.contexto = 'DESTETE' AND p.peso > 0
      `).get(v.rp);
      
      stats[v.rp] = {
        pva: pvaRow?.pva || null,
        cc_prom: ccRow?.cc_prom || null,
        peso_dest_crias: hijosDestete?.prom_dest || null,
        n_crias_destete: hijosDestete?.n || 0
      };
    }
