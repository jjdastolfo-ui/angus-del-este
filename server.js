...
app.get("/api/repro/toros", (req, res) => {
  ...
  res.json(db.prepare("SELECT * FROM repro_toros WHERE activo=1 ORDER BY tipo, nombre").all());
});

app.post("/api/repro/toros", (req, res) => {
  if (!reproOk(res)) return;
  try { res.json({ ok: true, id: reproduccion.guardarToro(db, req.body) }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});

app.get("/api/repro/protocolos", (req, res) => {
  if (!reproOk(res)) return;
  const lista = db.prepare("SELECT id FROM repro_protocolos ORDER BY temporada DESC, id DESC").all();
  res.json(lista.map(p => reproduccion.estado(db, p.id)));
});

app.get("/api/repro/protocolos/:id", (req, res) => {
  if (!reproOk(res)) return;
  const e = reproduccion.estado(db, req.params.id);
  if (!e) return res.status(404).json({ error: "No existe" });
  e.vacas = db.prepare("SELECT * FROM repro_vacas WHERE protocolo_id=? ORDER BY rp").all(req.params.id);
  res.json(e);
});

app.post("/api/repro/protocolos", (req, res) => {
  if (!reproOk(res)) return;
  try { res.json({ ok: true, id: reproduccion.crearProtocolo(db, req.body) }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});
...
app.post("/api/repro/protocolos/:id/vacas", (req, res) => {
  ...
  const animales = rps.map(rp => db.prepare("SELECT id, rp FROM animales WHERE upper(rp)=upper(?)").get(rp)).filter(Boolean);
  res.json({ ok: true, agregadas: reproduccion.agregarVacas(db, req.params.id, animales), pedidas: rps.length });
});

app.post("/api/repro/protocolos/:id/ejecucion", (req, res) => {
  if (!reproOk(res)) return;
  res.json(reproduccion.registrarEjecucion(db, req.params.id, req.body));
});

app.post("/api/repro/protocolos/:id/eco-precoz", (req, res) => {
  if (!reproOk(res)) return;
  res.json(reproduccion.registrarEcoPrecoz(db, req.params.id, req.body.resultados || [], req.body.fecha));
});

app.post("/api/repro/protocolos/:id/eco-final", (req, res) => {
  if (!reproOk(res)) return;
  res.json(reproduccion.registrarEcoFinal(db, req.params.id, req.body.resultados || [], req.body.fecha));
});
...
app.post("/api/repro/protocolos/:id/iatf", async (req, res) => {
  ...
  const r = await reproduccion.ejecutarIATF(db, req.params.id, req.body,
...
app.get("/api/repro/protocolos/:id/items", (req, res) => {
  ...
  const out = {};
  etapas.forEach(e => { out[e] = reproduccion.itemsDeEtapa(db, req.params.id, e); });
  res.json({ items: out, etapas: reproduccion.ETAPAS_COSTO, costos: reproduccion.costosProtocolo(db, req.params.id) });
});

app.post("/api/repro/protocolos/:id/items", (req, res) => {
  if (!reproOk(res)) return;
  try { res.json({ ok: true, id: reproduccion.guardarItem(db, req.params.id, req.body) }); }
  catch (e) { res.status(400).json({ error: e.message }); }
});

app.delete("/api/repro/items/:id", (req, res) => {
  if (!reproOk(res)) return;
  res.json(reproduccion.borrarItem(db, req.params.id));
});
...
app.post("/api/repro/protocolos/:id/ejecutar/:etapa", async (req, res) => {
  if (!reproOk(res)) return;
  const campoKey = req.body.campo || req.query.campo || CAMPO_DEFAULT;
  const r = await reproduccion.ejecutarEtapa(db, req.params.id, req.params.etapa.toUpperCase(), req.body,
    (producto, cant, detalle, fecha) => consumirStock(campoKey, producto, cant, detalle, fecha));
  res.status(r.ok ? 200 : 400).json(r);
});
...
app.get("/api/repro/partos", (req, res) => {
  if (!reproOk(res)) return;
  res.json(db.prepare(`SELECT p.*, t.nombre AS padre_nombre FROM repro_partos p
...
app.post("/api/repro/partos", (req, res) => {
  if (!reproOk(res)) return;
  const r = reproduccion.registrarParto(db, req.body);
  res.status(r.ok ? 200 : 400).json(r);
});

app.post("/api/repro/partos/:id/padre", (req, res) => {
  if (!reproOk(res)) return;
  res.json(reproduccion.corregirPadre(db, req.params.id, req.body.toro_id, req.body.quien, req.body.motivo));
});
