...
function esConsultaLluvia(body) {
  ...
  if (!/(lluvia|llovi|precipitac|pluviom|mil[ií]metro|\bmm\b)/.test(b)) return false;
  if (detectarLluvia(body)) return false;              // trae mm: es un registro
  return /(cu[aá]nt|cuant|llevamos|acumulad|total|hubo|hay|van|[uú]ltim|promedio|compar|mes|a[ñn]o|momento|ver|mostr|pas[aá]me|dame)/.test(b)
      || /^(lluvias?|precipitaciones)\??$/.test(b.trim());
}
...
app.post("/webhook-interno", async (req, res) => {
  try {
    ...
    // ── INTERCEPT 0: LLUVIA → diario de IMPROLUX (punto compartido de campo) ──
    {
      ...
      if (esConsultaLluvia(body)) {
        return res.json({ respuesta: await consultarLluvia(campoKey, body) });
      }
      const ll = detectarLluvia(body);
      if (ll) {
        const resp = await registrarLluviaImprolux(ll.mm, campoKey, "", ll.fecha);
