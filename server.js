...
function ejecutarAccion(accion) {
  ...
  if (accion.accion === "descontar_consumo_diario") {
    ...
    for (const c of consumos) {
      ...
      total += consumoDia;
      const al = db.prepare("SELECT nombre,stock_actual FROM stock_alimento WHERE id = ?").get(c.alimento_id);
      detalles.push(`  ${al.nombre} → -${consumoDia}kg (quedan ${al.stock_actual.toFixed(0)}kg)`);
    }
    return `✅ Consumo del día descontado: ${total.toFixed(1)}kg\n${detalles.join('\n')}`;
  }
