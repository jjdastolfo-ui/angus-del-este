...
async function resolverCampo(campoKey) {
  ...
  try {
    ...
    if (Array.isArray(lista) && lista.length) {
      ...
      const elegido = exacto ? exacto.nombre : lista[0].nombre;
      if (!exacto) console.log(`[diario] "${campo}" no existe en el financiero, uso "${elegido}"`);
      _campoCache[campoKey] = { campo: elegido, t: Date.now() };
      return { url, campo: elegido };
    }
  } catch (e) {}
  return { url, campo };
}
