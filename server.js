...
app.post("/api/recalcular", (req, res) => {
  try {
    ...
    let contextos = 0, gdps = 0;
    
    for (const p of pesadas) {
      // Recalcular contexto (excepto NACIMIENTO que es manual)
      if (p.contexto !== 'NACIMIENTO' && p.fecha_nac) {
    ...
    // Recalcular GDP para cada animal
    const animales = db.prepare("SELECT DISTINCT animal_id FROM pesadas").all();
    for (const { animal_id } of animales) {
      const gdp = calcularGDP(animal_id);
      if (gdp !== null) {
        // Poner GDP en la última pesada
        const ultima = db.prepare("SELECT id FROM pesadas WHERE animal_id = ? ORDER BY fecha DESC LIMIT 1").get(animal_id);
        if (ultima) {
          db.prepare("UPDATE pesadas SET gdp = ? WHERE id = ?").run(gdp, ultima.id);
          gdps++;
        }
      }
    }
    
    res.json({ mensaje: `✅ Recalculado: ${contextos} contextos corregidos, ${gdps} GDP actualizados` });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});
