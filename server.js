...
const CAMPOS = {
  ...
  videla: {
    ...
    codigo: "",
    raza: "A. ANGUS",
    whatsapp: process.env.WHATSAPP_VIDELA || "",
    admin: process.env.ADMIN_VIDELA || "",
...
// Cliente Videla (cuenta secundaria)
if (process.env.TWILIO_SID_VIDELA && process.env.TWILIO_TOKEN_VIDELA) {
  twilioClients.videla = twilio(process.env.TWILIO_SID_VIDELA, process.env.TWILIO_TOKEN_VIDELA);
  console.log("Twilio Videla configurado");
}
...
// Para APIs REST: ?campo=angus_del_este o ?campo=videla
...
function ejecutarAccion(accion) {
  ...
  if (accion.accion === "programar_consumo") {
    ...
    const dias = al.stock_actual > 0 ? Math.floor(al.stock_actual / consumoDia) : 0;
    return `✅ Consumo programado: ${cantidad_animales} animales × ${kg_por_animal_dia}kg/día = ${consumoDia}kg/día\n📦 Stock: ${al.stock_actual}kg → alcanza para ${dias} días`;
  }
