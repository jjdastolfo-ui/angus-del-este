...
app.get("/api/stats/vacas", (req, res) => {
  try {
    ...
    for (const v of vacas) {
      // PVA: promedio de pesadas ADULTA. Si no hay, usar último peso (cualquier contexto)
      let pvaRow = db.prepare(`
        SELECT AVG(peso) as pva FROM pesadas 
        WHERE animal_id = ? AND contexto = 'ADULTA'
      `).get(v.id);
      if (!pvaRow?.pva) {
        // Fallback: último peso registrado (que no sea NACIMIENTO ni DESTETE de cuando era ternera)
        pvaRow = db.prepare(`
          SELECT peso as pva FROM pesadas 
          WHERE animal_id = ? AND contexto NOT IN ('NACIMIENTO','DESTETE')
      ...
      // Buscar con match flexible: madre_rp exacto O sin prefijo ADE O con prefijo ADE
      const rp = v.rp;
