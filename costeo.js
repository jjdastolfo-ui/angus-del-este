...
function crearTablas(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS costeo_lotes_campo (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      nombre TEXT NOT NULL UNIQUE,          -- igual al de IMPROLUX
...
function motor(db) {
  ...
  function distribucionLote(loteId, fecha) {
    ...
    const tmp = db.prepare(`SELECT * FROM costeo_lote_sector
      ...
      .get(loteId, f, f);
    if (tmp) return normalizarPct(tmp) || (tmp.sector ? { [tmp.sector]: 1 } : { CRIA: 1 });
    const l = db.prepare("SELECT * FROM costeo_lotes_campo WHERE id = ?").get(loteId);
    if (!l) return { CRIA: 1 };
    if (l.sector === "EXCLUIDO") return {};
...
function rutas(app, motores, CAMPO_DEFAULT) {
    ...
    try {
      ...
      if (b.id) {
        ...
        db2.prepare(`UPDATE costeo_lotes_campo SET nombre=?, ha_totales=?, ha_sembrables=?,
          kg_ha_renta=?, sector=?, pct_cria=?, pct_recria=?, pct_corral=?, activo=?, notas=? WHERE id=?`)
          .run(b.nombre, parseFloat(b.ha_totales) || 0, parseFloat(b.ha_sembrables) || 0,
               parseFloat(b.kg_ha_renta) || 0, dom, pc, pr, pk,
  ...
  /* Trae los lotes de IMPROLUX con sus hectáreas */
  app.post("/api/costeo/tierra/sync-improlux", async (req, res) => {
    try {
      ...
      if (!Array.isArray(lotes)) throw new Error("Respuesta inesperada");
      const db2 = D(req);
      const kgDef = parseFloat(M(req).cfg("kg_ha_default", "35")) || 35;
      let nuevos = 0, act = 0;
      const tx = db2.transaction(() => {
        lotes.forEach(l => {
          const ex = db2.prepare("SELECT id FROM costeo_lotes_campo WHERE UPPER(TRIM(nombre)) = UPPER(TRIM(?))").get(l.nombre);
          const ha = l.hectareas || 0, hs = l.ha_sembrables || 0;
          if (ex) {
            db2.prepare("UPDATE costeo_lotes_campo SET ha_totales = ?, ha_sembrables = ? WHERE id = ?").run(ha, hs, ex.id);
            act++;
          } else {
            /* La isla paga menos renta por la forestación */
            const forestal = /ISLA|FOREST|EUCALIPT/i.test(l.nombre || "");
            db2.prepare(`INSERT INTO costeo_lotes_campo (nombre,ha_totales,ha_sembrables,kg_ha_renta,sector,notas)
              VALUES (?,?,?,?,?,?)`).run(String(l.nombre).trim(), ha, hs,
              forestal ? 7 : kgDef, "CRIA", forestal ? "Renta menor por la forestación" : null);
            db2.prepare("UPDATE costeo_lotes_campo SET pct_cria = 100 WHERE UPPER(TRIM(nombre)) = UPPER(TRIM(?))").run(String(l.nombre).trim());
            nuevos++;
          }
        });
      });
      tx();
      ok(res, { nuevos, actualizados: act, total: lotes.length });
    } catch (e) { err(res, e); }
  });
    ...
    try {
      ...
      const tx = db2.transaction(() => {
        for (const o of ordenes) {
          ...
          if (!tipo || !VIDA_TIPO_IMP[tipo]) { res2.sin_tipo++; continue; }
          if (db2.prepare("SELECT 1 FROM costeo_implantaciones WHERE orden_improlux = ?").get(o.id)) {
            res2.ya_estaban++; continue;
          }
          const costo = parseFloat(o.total_ejecutado) || parseFloat(o.total_planificado) || 0;
          if (!(costo > 0)) continue;
          ...
          const lotes = nombres.map(n =>
            db2.prepare("SELECT * FROM costeo_lotes_campo WHERE UPPER(TRIM(nombre)) = UPPER(TRIM(?))").get(n)
          ).filter(Boolean);
          ...
          for (const l of lotes) {
            ...
            ins.run(l.id, tipo, parseInt(o.anio, 10) || new Date().getUTCFullYear(),
              costo * parte, l.ha_totales || 0, VIDA_TIPO_IMP[tipo], o.id,
              `OT ${o.numero || o.id}/${o.anio || ""} · ${o.titulo || ""}`.trim());
          }
          res2.importadas++;
