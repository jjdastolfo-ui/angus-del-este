...
function crearTablas(db) {
  db.exec(`
    ...
    -- Datos genéticos de cada producto de stock. IMPROLUX guarda nombre,
    -- cantidad y costo; acá van RP, HBU y raza, que son de la cabaña.
    -- El HBU es el registro en ARU: sin él no hay Breedplan ni catálogo.
    CREATE TABLE IF NOT EXISTS costeo_genetica (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      producto TEXT NOT NULL UNIQUE,        -- nombre del producto en IMPROLUX
      tipo TEXT DEFAULT 'SEMEN',            -- SEMEN / EMBRION
      padre_rp TEXT, padre_nombre TEXT, padre_hbu TEXT,
      madre_rp TEXT, madre_nombre TEXT, madre_hbu TEXT,   -- solo embriones
      raza TEXT DEFAULT 'ABERDEEN ANGUS',
      registro TEXT,                        -- PP / SA
      proveedor TEXT, notas TEXT
    );

    -- Programación de servicios. Se arma con anticipación: a qué vientre le va
    -- qué semen, en qué temporada. Sirve para saber si las dosis alcanzan
    -- ANTES de comprar, no el día del trabajo.
    CREATE TABLE IF NOT EXISTS costeo_plan_servicio (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      animal_id INTEGER NOT NULL,
      temporada TEXT,
      fecha_prevista TEXT,
      tipo TEXT DEFAULT 'IATF',        -- IATF / EMBRION / TORO / TORO_REPASO
      producto TEXT,                    -- semen o embrión del stock
      toro_rp TEXT,                     -- si es servicio natural
      toro_nombre TEXT,                 -- toro que todavía no está cargado
      por_comprar INTEGER DEFAULT 0,    -- hay que comprarlo
      grupo TEXT,                       -- cabeza / cuerpo / cola del repaso
      estado TEXT DEFAULT 'PLANIFICADO',-- PLANIFICADO / EJECUTADO / CANCELADO
      fecha_ejecucion TEXT,
      notas TEXT,
      created_at TEXT DEFAULT (datetime('now')),
      UNIQUE(animal_id, temporada)
    );
  ...
  [
    "ALTER TABLE costeo_plan_servicio ADD COLUMN toro_nombre TEXT",
    "ALTER TABLE costeo_plan_servicio ADD COLUMN por_comprar INTEGER DEFAULT 0",
    "ALTER TABLE costeo_plan_servicio ADD COLUMN grupo TEXT",
    "ALTER TABLE costeo_genetica ADD COLUMN notas TEXT",
    "ALTER TABLE costeo_lotes_campo ADD COLUMN nombres_previos TEXT",
    "ALTER TABLE costeo_estructura ADD COLUMN pct_ganaderia REAL DEFAULT 100"
  ].forEach(q => { try { db.exec(q); } catch (e) {} });
...
function motor(db) {
  ...
  const clasificarProducto = x => {
    ...
    if (["ALIMENTO", "VETERINARIO", "AGRICOLA", "GENETICA"].includes(r)) return r;
    const t = (x.nombre + " " + (x.categoria || "")).toUpperCase();
    if (/SEMEN|PAJUELA|EMBRION|EMBRIÓN|DOSIS\s*SEMEN/.test(t)) return "GENETICA";
  ...
  async function sincronizarPreciosImprolux(baseUrl) {
    ...
    const res = { ruta: r.ruta, leidos: r.items.length, alimentos: 0, sanidad: 0, genetica: 0, detalle: [] };
    ...
    const tx = db.transaction(() => {
      for (const it of r.items) {
        ...
        else if (tipo === "GENETICA") res.genetica = (res.genetica || 0) + 1;
        else res.agricolas++;
  ...
  function costearIATF(o) {
    ...
    /* Si el semen sale del stock de IMPROLUX, el precio lo pone el stock y no
       hace falta escribirlo. El descuento de la dosis lo hace el endpoint. */
  ...
  /* ── DESHACER ──────────────────────────────────────────────────────────────
     Un tacto mal cargado, una evaluación con el destino equivocado. Borra ese
     asiento y todo lo que vino después, porque lo posterior se apoyó en él.
     Deja al animal en la etapa anterior, listo para volver a registrarlo. */

  function previsualizarDeshacer(ref, asientoId) {
    const a = resolver(ref);
    const asi = db.prepare("SELECT * FROM costeo_kgne WHERE id = ? AND animal_id = ?").get(asientoId, a.id);
    if (!asi) throw new Error("Ese movimiento no es de este animal");

    /* Se borra por orden de REGISTRO, no de fecha: la apertura de inventario
       lleva la fecha de hoy pero se cargó antes que un tacto de marzo, y por
       fecha se la llevaba puesta. El id refleja lo que se hizo primero. */
    const posteriores = db.prepare(
      "SELECT * FROM costeo_kgne WHERE animal_id = ? AND id >= ? ORDER BY id").all(a.id, asi.id);
    const previo = db.prepare(
      "SELECT * FROM costeo_kgne WHERE animal_id = ? AND id < ? ORDER BY fecha DESC, id DESC LIMIT 1")
      .get(a.id, asi.id);

    return {
      rp: a.rp, asiento: asi, borra: posteriores.length,
      detalle: posteriores.map(x => ({ id: x.id, fecha: x.fecha, tipo: x.tipo,
        concepto: x.concepto, produccion: Math.round(x.kgne_produccion) })),
      queda_en: previo ? { etapa: previo.etapa, saldo: Math.round(previo.kgne_saldo), fecha: previo.fecha }
                       : { etapa: "sin ficha", saldo: 0 },
      revive: asi.tipo === "BAJA" || posteriores.some(x => x.tipo === "BAJA"),
      produccion_que_se_va: Math.round(posteriores.reduce((s, x) => s + (x.kgne_produccion || 0), 0))
    };
  }

  function deshacerDesde(ref, asientoId) {
    const prev = previsualizarDeshacer(ref, asientoId);
    const a = resolver(ref);
    const asi = db.prepare("SELECT * FROM costeo_kgne WHERE id = ?").get(asientoId);

    const tx = db.transaction(() => {
      db.prepare("DELETE FROM costeo_kgne WHERE animal_id = ? AND id >= ?").run(a.id, asi.id);
      /* Las etapas que se cerraron desde esa fecha también se van */
      db.prepare("DELETE FROM costeo_etapas WHERE animal_id = ? AND fecha_fin >= ?").run(a.id, asi.fecha);
      /* Si había una baja, el animal vuelve al rodeo */
      if (prev.revive) {
        db.prepare("UPDATE animales SET estado='ACTIVO', fecha_salida=NULL, motivo_salida=NULL WHERE id = ?").run(a.id);
        db.prepare("UPDATE costeo_permanencia SET fecha_hasta = NULL WHERE animal_id = ? AND fecha_hasta = ?")
          .run(a.id, asi.fecha);
      }
    });
    tx();
    return { ...prev, deshecho: true,
      aviso: prev.revive ? "El animal volvió a ACTIVO. Si estaba vendido, revisá el lote." : null };
  }


  /* ── GENÉTICA ──────────────────────────────────────────────────────────────
     El costo es lo de menos: lo que importa es que al inseminar quede quién es
     el padre con su HBU, para que el ternero salga bien catalogado después. */

  function listarGenetica() {
    /* El nombre sale SIEMPRE del producto de IMPROLUX. Antes venía de g.*, que
       es NULL mientras no exista la ficha genética: el modal abría sin nombre y
       el guardado se iba contra una clave vacía. */
    return db.prepare(`SELECT
        p.producto            AS producto,
        p.costo_unitario, p.stock_improlux, p.unidad, p.actualizado,
        g.id AS genetica_id, g.tipo, g.padre_rp, g.padre_nombre, g.padre_hbu,
        g.madre_rp, g.madre_nombre, g.madre_hbu, g.raza, g.registro, g.proveedor, g.notas
      FROM costeo_productos p
      LEFT JOIN costeo_genetica g ON UPPER(TRIM(g.producto)) = UPPER(TRIM(p.producto))
      WHERE p.tipo = 'GENETICA' ORDER BY p.producto`).all().map(x => ({
        ...x,
        tipo: x.tipo || (/EMBRION|EMBRIÓN/i.test(x.producto || "") ? "EMBRION" : "SEMEN"),
        valor: (x.stock_improlux || 0) * (x.costo_unitario || 0),
        completo: !!(x.padre_hbu && x.padre_nombre)
      }));
  }

  function guardarGenetica(o) {
    const prod = String(o.producto || "").trim();
    if (!prod) throw new Error("Falta el nombre del producto");
    /* Tiene que existir en el stock: si no, es un nombre mal copiado */
    const enStock = db.prepare(
      "SELECT 1 FROM costeo_productos WHERE UPPER(TRIM(producto)) = UPPER(TRIM(?))").get(prod);
    if (!enStock) throw new Error(`"${prod}" no está en el stock. Traelo de IMPROLUX primero.`);
    o = { ...o, producto: prod };
    const tipo = String(o.tipo || "SEMEN").toUpperCase();
    /* En un embrión hacen falta las dos líneas; en una pajuela, solo el padre */
    db.prepare(`INSERT INTO costeo_genetica
      (producto,tipo,padre_rp,padre_nombre,padre_hbu,madre_rp,madre_nombre,madre_hbu,raza,registro,proveedor,notas)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
      ON CONFLICT(producto) DO UPDATE SET tipo=excluded.tipo,
        padre_rp=excluded.padre_rp, padre_nombre=excluded.padre_nombre, padre_hbu=excluded.padre_hbu,
        madre_rp=excluded.madre_rp, madre_nombre=excluded.madre_nombre, madre_hbu=excluded.madre_hbu,
        raza=excluded.raza, registro=excluded.registro, proveedor=excluded.proveedor, notas=excluded.notas`)
      .run(prod, tipo,
        o.padre_rp || null, o.padre_nombre || null, o.padre_hbu || null,
        tipo === "EMBRION" ? (o.madre_rp || null) : null,
        tipo === "EMBRION" ? (o.madre_nombre || null) : null,
        tipo === "EMBRION" ? (o.madre_hbu || null) : null,
        o.raza || "ABERDEEN ANGUS", o.registro || null, o.proveedor || null, o.notas || null);
    return { guardado: prod, tipo,
             padre: [o.padre_nombre, o.padre_hbu ? "HBU " + o.padre_hbu : null].filter(Boolean).join(" · ") || null };
  }

  /* Trae los datos del padre desde el plantel propio: si el toro está cargado
     como animal, no hace falta escribir el nombre ni el HBU de nuevo. */
  function padreDesdePlantel(ref) {
    const a = animal(ref);
    if (!a) return null;
    return { padre_rp: a.rp, padre_nombre: a.nombre || null, padre_hbu: a.hbu || null,
             raza: a.raza || null, registro: a.registro || null };
  }

  const genDe = producto => db.prepare(
    "SELECT * FROM costeo_genetica WHERE UPPER(TRIM(producto)) = UPPER(TRIM(?))").get(producto || "");


  /* ── PROGRAMACIÓN DE SERVICIOS ─────────────────────────────────────────────
     Asignar de antemano qué semen le toca a cada vientre. Lo importante no es
     la lista: es saber si las dosis alcanzan antes de que llegue el día. */

  const esToro = t => ["TORO", "TORO_REPASO"].includes(t);

  function programarServicio(o) {
    const refs = o.animales || [];
    if (!refs.length) throw new Error("Sin animales");
    const temporada = o.temporada || cicloDe(o.fecha_prevista || hoy());
    const tipo = String(o.tipo || "IATF").toUpperCase();
    if (!esToro(tipo) && !o.producto) throw new Error("Elegí el semen o el embrión");
    if (esToro(tipo) && !o.toro_rp && !o.toro_nombre) throw new Error("Elegí el toro");

    /* El repaso puede ir con un toro que todavía no está cargado, o con uno
       que hay que comprar. En ese caso alcanza el nombre. */
    const ins = db.prepare(`INSERT INTO costeo_plan_servicio
      (animal_id,temporada,fecha_prevista,tipo,producto,toro_rp,toro_nombre,por_comprar,grupo,notas)
      VALUES (?,?,?,?,?,?,?,?,?,?)
      ON CONFLICT(animal_id,temporada) DO UPDATE SET
        fecha_prevista=excluded.fecha_prevista, tipo=excluded.tipo,
        producto=excluded.producto, toro_rp=excluded.toro_rp,
        toro_nombre=excluded.toro_nombre, por_comprar=excluded.por_comprar,
        grupo=excluded.grupo, notas=excluded.notas, estado='PLANIFICADO'`);

    const res = { temporada, tipo, producto: o.producto || o.toro_rp,
                  programados: 0, no_encontrados: [], machos: [] };
    const tx = db.transaction(() => {
      for (const r of refs) {
        const a = animal(r);
        if (!a) { res.no_encontrados.push(r); continue; }
        /* Un macho no recibe servicio: casi siempre es un error de selección */
        if (esM(a.sexo)) { res.machos.push(a.rp); continue; }
        ins.run(a.id, temporada, o.fecha_prevista || null, tipo,
                esToro(tipo) ? null : (o.producto || null),
                o.toro_rp || null, o.toro_nombre || null,
                o.por_comprar ? 1 : 0, o.grupo || null, o.notas || null);
        res.programados++;
      }
    });
    tx();
  ...
  function listarPlan(o) {
    const op = o || {};
    const temporada = op.temporada || cicloDe(hoy());
    const filas = db.prepare(`SELECT p.*, a.rp, a.nombre, a.categoria, a.registro, a.fecha_nac
      FROM costeo_plan_servicio p JOIN animales a ON a.id = p.animal_id
      WHERE p.temporada = ? AND a.estado = 'ACTIVO'
      ORDER BY p.producto, p.toro_rp, a.rp`).all(temporada);

    /* Por producto: cuántas dosis hacen falta y cuántas hay */
    const porProducto = {};
    for (const f of filas) {
      const etiqToro = f.toro_nombre || (f.toro_rp ? "Toro " + f.toro_rp : null);
      const k = f.producto || etiqToro || "sin asignar";
      if (!porProducto[k]) {
        const st = db.prepare(
          "SELECT costo_unitario, stock_improlux FROM costeo_productos WHERE UPPER(TRIM(producto)) = UPPER(TRIM(?))")
          .get(f.producto || "");
        const g = f.producto ? genDe(f.producto) : null;
        /* Si el toro está cargado como animal, se muestra su nombre y HBU */
        let pedToro = null;
        if (esToro(f.tipo) && f.toro_rp) {
          const t = animal(f.toro_rp);
          if (t) pedToro = [t.nombre, t.hbu ? "HBU " + t.hbu : null].filter(Boolean).join(" · ") || null;
        }
        porProducto[k] = { clave: k, tipo: f.tipo, producto: f.producto,
          toro_rp: f.toro_rp, toro_nombre: f.toro_nombre, por_comprar: !!f.por_comprar,
          padre: g ? [g.padre_nombre, g.padre_hbu ? "HBU " + g.padre_hbu : null].filter(Boolean).join(" · ") : pedToro,
          planificados: 0, ejecutados: 0,
          /* Un toro no tiene stock de dosis: cubre un rodeo entero */
          stock: esToro(f.tipo) ? null : (st ? st.stock_improlux : null),
          costo_unitario: esToro(f.tipo) ? null : (st ? st.costo_unitario : null) };
      }
      porProducto[k].planificados++;
      if (f.estado === "EJECUTADO") porProducto[k].ejecutados++;
    }
    const resumen = Object.values(porProducto).map(x => {
      const pendientes = x.planificados - x.ejecutados;
      const faltan = x.stock === null ? null : Math.max(0, pendientes - x.stock);
      return { ...x, pendientes, faltan,
        sobran: x.stock === null ? null : Math.max(0, x.stock - pendientes),
        /* Sin nada en stock es una compra entera, no un faltante parcial */
        a_comprar: x.stock === 0 && pendientes > 0,
        costo_previsto: x.costo_unitario ? Math.round(pendientes * x.costo_unitario * 100) / 100 : null,
        costo_faltante: x.costo_unitario && faltan ? Math.round(faltan * x.costo_unitario * 100) / 100 : null };
    });

    return { temporada, animales: filas.length,
      planificados: filas.filter(f => f.estado === "PLANIFICADO").length,
      ejecutados: filas.filter(f => f.estado === "EJECUTADO").length,
      costo_previsto: Math.round(resumen.reduce((s, x) => s + (x.costo_previsto || 0), 0) * 100) / 100,
      costo_a_comprar: Math.round(resumen.reduce((s, x) => s + (x.costo_faltante || 0), 0) * 100) / 100,
      dosis_a_comprar: resumen.reduce((s, x) => s + (x.faltan || 0), 0),
      resumen, detalle: filas };
  }

  /* Los vientres activos que todavía no tienen nada asignado */
  function sinProgramar(temporada) {
    const t = temporada || cicloDe(hoy());
    return db.prepare(`SELECT a.rp, a.nombre, a.categoria, a.fecha_nac FROM animales a
      WHERE a.estado = 'ACTIVO' AND UPPER(a.sexo) LIKE 'H%'
        AND a.categoria IN ('VACA','VAQUILLONA')
        AND NOT EXISTS (SELECT 1 FROM costeo_plan_servicio p
                        WHERE p.animal_id = a.id AND p.temporada = ?)
      ORDER BY a.categoria, a.rp`).all(t);
  }

  function borrarPlan(o) {
    const op = o || {};
    if (op.id) { db.prepare("DELETE FROM costeo_plan_servicio WHERE id = ?").run(op.id); return { borrados: 1 }; }
    if (op.temporada && op.producto) {
      const r = db.prepare("DELETE FROM costeo_plan_servicio WHERE temporada = ? AND producto = ? AND estado = 'PLANIFICADO'")
        .run(op.temporada, op.producto);
      return { borrados: r.changes };
    }
    if (op.temporada) {
      const r = db.prepare("DELETE FROM costeo_plan_servicio WHERE temporada = ? AND estado = 'PLANIFICADO'").run(op.temporada);
      return { borrados: r.changes };
    }
    throw new Error("Falta indicar qué borrar");
  }

  /* Marca como ejecutado lo que se hizo. Lo llama la IATF real. */
  function marcarPlanEjecutado(animalIds, temporada, fecha) {
    const t = temporada || cicloDe(fecha || hoy());
    const up = db.prepare(`UPDATE costeo_plan_servicio SET estado='EJECUTADO', fecha_ejecucion=?
      WHERE animal_id = ? AND temporada = ? AND estado='PLANIFICADO'`);
    let n = 0;
    const tx = db.transaction(() => { animalIds.forEach(id => { n += up.run(fecha || hoy(), id, t).changes; }); });
    tx();
    return n;
  }


  /* Los toros disponibles para servicio natural, con nombre y HBU */
  function torosPlantel() {
    /* El repaso se hace con toros propios: los de plantel y las recrías que
       van a quedar. Los de venta no, porque se van al remate. */
    return db.prepare(`SELECT rp, nombre, hbu, registro, categoria, fecha_nac FROM animales
      WHERE estado = 'ACTIVO' AND UPPER(sexo) LIKE 'M%'
        AND UPPER(COALESCE(categoria,'')) IN ('TORO','RECRIA')
        AND UPPER(COALESCE(destino,'PLANTEL')) <> 'VENTA'
      ORDER BY nombre, rp`).all().map(t => {
        const edad = t.fecha_nac ? Math.round(anios(t.fecha_nac, hoy()) * 10) / 10 : null;
        return { ...t, edad,
          /* Menos de 15 meses no sirve para servicio todavía */
          joven: edad !== null && edad < 1.25,
          etiqueta: [t.nombre, "RP " + t.rp, t.hbu ? "HBU " + t.hbu : null].filter(Boolean).join(" · ") };
      });
  }


  /* Crear una pajuela o embrión que TODAVÍA NO SE COMPRÓ. Se da de alta en
     IMPROLUX con cantidad cero: el producto existe, tiene sus datos, y cuando
     llegue la compra solo se carga la cantidad. Sirve para programar contra
     algo que aún no está: el sistema dice cuántas dosis hacen falta. */
  async function crearGenetica(o) {
    const nombre = String(o.producto || o.nombre || "").trim().toUpperCase();
    if (!nombre) throw new Error("Falta el nombre");
    const tipo = String(o.tipo || "SEMEN").toUpperCase();

    const yaEsta = db.prepare(
      "SELECT 1 FROM costeo_productos WHERE UPPER(TRIM(producto)) = UPPER(TRIM(?))").get(nombre);

    let enImprolux = false, avisoImprolux = null;
    if (!yaEsta) {
      const url0 = (process.env.IMPROLUX_URL || "https://improlux-bot-production.up.railway.app").replace(/\/$/, "");
      try {
        const resp = await fetch(url0 + "/api/stock", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ nombre, rubro: "GENETICA",
            categoria: tipo === "EMBRION" ? "EMBRION" : "SEMEN",
            unidad: tipo === "EMBRION" ? "u" : "dosis",
            cantidad: 0, precio_unitario: parseFloat(o.precio_estimado) || 0,
            notas: o.notas || "Creado desde ADE para programar · sin comprar" })
        });
        const d = await resp.json().catch(() => ({}));
        if (d.id || d.nombre) enImprolux = true;
        else avisoImprolux = d.error || "IMPROLUX no lo aceptó";
      } catch (e) { avisoImprolux = "Sin conexión con IMPROLUX"; }

      /* El espejo local se crea igual, así se puede programar de una */
      db.prepare(`INSERT OR IGNORE INTO costeo_productos
        (producto,tipo,unidad,costo_unitario,stock_improlux,actualizado)
        VALUES (?,?,?,?,0,datetime('now'))`)
        .run(nombre, "GENETICA", tipo === "EMBRION" ? "u" : "dosis",
             parseFloat(o.precio_estimado) || 0);
    }

    guardarGenetica({ ...o, producto: nombre, tipo });
    return { producto: nombre, tipo, ya_existia: !!yaEsta,
             en_improlux: enImprolux, aviso: avisoImprolux,
             precio_estimado: parseFloat(o.precio_estimado) || 0 };
  }
  ...
  return { precios, hito, animal, saldo, etapaActual, asiento, cerrarEtapa,
           ...
           previsualizarDeshacer, deshacerDesde,
           listarGenetica, guardarGenetica, padreDesdePlantel, genDe,
           programarServicio, listarPlan, sinProgramar, borrarPlan, marcarPlanEjecutado,
           torosPlantel, crearGenetica };
}
...
function rutas(app, motores, CAMPO_DEFAULT) {
  ...
  app.get("/api/costeo/deshacer/:ref/:asiento", (req, res) => {
    try { ok(res, M(req).previsualizarDeshacer(req.params.ref, parseInt(req.params.asiento, 10))); }
    catch (e) { err(res, e); }
  });
  app.delete("/api/costeo/deshacer/:ref/:asiento", (req, res) => {
    try { ok(res, M(req).deshacerDesde(req.params.ref, parseInt(req.params.asiento, 10))); }
    catch (e) { err(res, e); }
  });

  /* ── PROGRAMACIÓN DE SERVICIOS ── */
  app.get("/api/costeo/plan", (req, res) => {
    try { ok(res, M(req).listarPlan(req.query)); } catch (e) { err(res, e); }
  });
  app.get("/api/costeo/plan/sin-programar", (req, res) => {
    try { ok(res, { animales: M(req).sinProgramar(req.query.temporada) }); } catch (e) { err(res, e); }
  });
  app.post("/api/costeo/plan", (req, res) => {
    try { ok(res, M(req).programarServicio(req.body || {})); } catch (e) { err(res, e); }
  });
  app.post("/api/costeo/plan/borrar", (req, res) => {
    try { ok(res, M(req).borrarPlan(req.body || {})); } catch (e) { err(res, e); }
  });

  app.post("/api/costeo/genetica/nueva", async (req, res) => {
    try { ok(res, await M(req).crearGenetica(req.body || {})); } catch (e) { err(res, e); }
  });

  app.get("/api/costeo/toros", (req, res) => {
    try { ok(res, { toros: M(req).torosPlantel() }); } catch (e) { err(res, e); }
  });

  /* ── GENÉTICA ── */
  app.get("/api/costeo/genetica", (req, res) => {
    try { ok(res, { genetica: M(req).listarGenetica() }); } catch (e) { err(res, e); }
  });
  app.post("/api/costeo/genetica", (req, res) => {
    try { ok(res, M(req).guardarGenetica(req.body || {})); } catch (e) { err(res, e); }
  });
  app.get("/api/costeo/padre/:ref", (req, res) => {
    const d = M(req).padreDesdePlantel(req.params.ref);
    if (!d) return err(res, new Error("No encontré ese animal en el plantel"));
    ok(res, d);
  });
  ...
  app.post("/api/costeo/iatf", async (req, res) => {
    try {
      ...
      /* El semen se descuenta del stock de genética: una dosis por vientre.
         Si el producto está en IMPROLUX, el costo sale de ahí. */
      let costoPajuela = parseFloat(b.costo_pajuela) || 0;
      let semenNota = "";
      let gen = null;
      if (b.semen_stock) {
        const pr = m.productoCache(b.semen_stock);
        if (pr && pr.costo_unitario > 0) costoPajuela = pr.costo_unitario;
        gen = m.genDe(b.semen_stock);
      }

      const base = m.costearIATF({ ...b, animales, costo_pajuela: costoPajuela,
        semen: b.semen_stock || b.semen });
      let extra = "", costoInsumos = 0;

      if (b.semen_stock) {
        const url0 = (process.env.IMPROLUX_URL || "https://improlux-bot-production.up.railway.app").replace(/\/$/, "");
        try {
          const resp = await fetch(url0 + "/api/stock/aplicar", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ producto: b.semen_stock, cantidad: animales.length,
              fecha: base.fecha, detalle: `IATF · ${animales.length} vientres` })
          });
          const d = await resp.json().catch(() => ({}));
          if (d.ok) {
            semenNota = `\n📦 Semen: -${animales.length} dosis de ${d.producto} (quedan ${d.restante})`;
            if (d.negativo) semenNota += " ⚠️ NEGATIVO";
          } else {
            semenNota = `\n⚠️ No pude descontar el semen del stock (${d.error || "no encontrado"}).`;
          }
        } catch (e) {
          semenNota = `\n⚠️ Sin conexión con IMPROLUX: el semen no se descontó.`;
        }
      }
      ...
      /* El padre queda registrado en cada vientre: es lo que después permite
         catalogar bien al ternero y cruzar los DEP. */
      if (gen && (gen.padre_rp || gen.padre_nombre)) {
        const ins = db2.prepare(`INSERT INTO servicios
          (animal_id, temporada, tipo_servicio, semen_iatf, fecha_iatf, created_at)
          VALUES (?,?,?,?,?,datetime('now'))`);
        const etiqueta = [gen.padre_nombre || gen.padre_rp, gen.padre_hbu ? "HBU " + gen.padre_hbu : null]
          .filter(Boolean).join(" · ");
        try {
          const tx = db2.transaction(() => {
            animales.forEach(a => ins.run(a.id, b.temporada || null, "IATF", etiqueta, base.fecha));
          });
          tx();
          base.lineas.push(`  • Padre registrado en ${animales.length} vientre(s): ${etiqueta}`);
        } catch (e) { base.lineas.push(`  • No pude registrar el servicio: ${e.message}`); }
      }

      try {
        const nPlan = m.marcarPlanEjecutado(animales.map(a => a.id), b.temporada, base.fecha);
        if (nPlan) base.lineas.push(`  • ${nPlan} vientre(s) marcados como ejecutados en la programación`);
      } catch (e) {}
