...
function motor(db) {
  ...
  function listarGenetica() {
    const p = precios();
    return db.prepare(`SELECT g.*, p.costo_unitario, p.stock_improlux, p.unidad, p.actualizado
      ...
      WHERE p.tipo = 'GENETICA' ORDER BY p.producto`).all().map(x => ({
        ...x, producto: x.producto || x.nombre,
  ...
  function guardarGenetica(o) {
    if (!o.producto) throw new Error("Falta el producto");
    ...
    db.prepare(`INSERT INTO costeo_genetica
      ...
      .run(String(o.producto).trim(), tipo,
