...
function crearTablas(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS costeo_plan_servicio (
      ...
      temporada TEXT,
      fecha_prevista TEXT,
      tipo TEXT DEFAULT 'IATF',        -- IATF / EMBRION / TORO
...
function motor(db) {
  ...
  function programarServicio(o) {
    ...
    if (tipo !== "TORO" && !o.producto) throw new Error("Elegí el semen o el embrión");
    if (tipo === "TORO" && !o.toro_rp) throw new Error("Elegí el toro");

    const ins = db.prepare(`INSERT INTO costeo_plan_servicio
      (animal_id,temporada,fecha_prevista,tipo,producto,toro_rp,notas)
      VALUES (?,?,?,?,?,?,?)
      ON CONFLICT(animal_id,temporada) DO UPDATE SET
        ...
        notas=excluded.notas, estado='PLANIFICADO'`);
    ...
    const tx = db.transaction(() => {
      for (const r of refs) {
        ...
        ins.run(a.id, temporada, o.fecha_prevista || null, tipo,
                o.producto || null, o.toro_rp || null, o.notas || null);
        res.programados++;
      }
    });
    tx();
  ...
  function listarPlan(o) {
    ...
    const porProducto = {};
    for (const f of filas) {
      const k = f.producto || (f.toro_rp ? "Toro " + f.toro_rp : "sin asignar");
      if (!porProducto[k]) {
        const st = db.prepare(
        ...
        porProducto[k] = { clave: k, tipo: f.tipo, producto: f.producto, toro_rp: f.toro_rp,
          padre: g ? [g.padre_nombre, g.padre_hbu ? "HBU " + g.padre_hbu : null].filter(Boolean).join(" · ") : null,
          planificados: 0, ejecutados: 0,
          stock: st ? st.stock_improlux : null,
          costo_unitario: st ? st.costo_unitario : null };
      }
    ...
    const resumen = Object.values(porProducto).map(x => {
      ...
      return { ...x, pendientes,
        faltan: x.stock === null ? null : Math.max(0, pendientes - x.stock),
