...
const POTREROS_DEFAULT = [
  ["Natural Amistad",   "NATURAL",  35, "ANUAL",     "Campo natural"],
  ["Pasturas Amistad",  "PASTURA",  85, "ANUAL",     "Pradera implantada"],
  ["Verdeos Amistad",   "VERDEO",   85, "SEMESTRAL", "Verdeo de invierno o verano"],
  ["Corrales Amistad",  "CORRAL",    0, "ANUAL",     "Sin renta — el costo es la ración"],
  ["Isla eucaliptus",   "FORESTAL",  7, "ANUAL",     "Campo natural forestal"]
];
...
function crearTablas(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS costeo_potrero_dias (
      ...
      fecha TEXT NOT NULL, animales INTEGER, ha REAL,
  ...
  const pot = db.prepare(`INSERT OR IGNORE INTO costeo_potreros
    (nombre,tipo,kg_carne_ha,periodo,notas) VALUES (?,?,?,?,?)`);
  POTREROS_DEFAULT.forEach(x => pot.run(x[0], x[1], x[2], x[3], x[4]));
...
function motor(db) {
  ...
  function sincronizar(opts) {
    ...
    if (o.potreros !== false) { try { res.potreros = aplicarPotreros(); } catch (e) {} }
  ...
  const kgAnualHa = p => (p.periodo === "SEMESTRAL" ? p.kg_carne_ha * 2 : p.kg_carne_ha);
  ...
  function aplicarPotreros(fecha) {
    ...
    const res = { fecha: f, asignaciones: asig.length, aplicadas: 0, kgne: 0, costo: 0, detalle: [] };
    for (const a of asig) {
      if (db.prepare("SELECT 1 FROM costeo_potrero_dias WHERE asignacion_id = ? AND fecha = ?").get(a.id, f)) continue;
      const n = animalesDelLote(a.lote_id).length;
      if (!n) continue;
      const ha = a.ha_aprovechables || a.ha_totales || 0;
      const kgneDia = ha * kgAnualHa(a) / 365;
      const costo = kgneDia * nov;
      db.prepare(`INSERT INTO costeo_potrero_dias (asignacion_id,potrero_id,lote_id,fecha,animales,ha,kgne_dia,novillo_pie,costo_total)
        VALUES (?,?,?,?,?,?,?,?,?)`).run(a.id, a.potrero_id, a.lote_id, f, n, ha, kgneDia, nov, costo);
    ...
    try {
      ...
      if (b.id) {
        D(req).prepare(`UPDATE costeo_potreros SET nombre=?, tipo=?, kg_carne_ha=?, periodo=?,
          ha_totales=?, ha_aprovechables=?, lote_improlux=?, activo=?, notas=? WHERE id=?`)
      ...
      const i = D(req).prepare(`INSERT INTO costeo_potreros
        (nombre,tipo,kg_carne_ha,periodo,ha_totales,ha_aprovechables,lote_improlux,notas)
