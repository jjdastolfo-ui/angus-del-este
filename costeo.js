...
function motor(db) {
  ...
  async function enviarConsumos(baseUrl) {
    ...
    for (const x of pend) {
      const r = await avisarConsumo(baseUrl, { producto: x.producto, cantidad: x.kg,
        fecha: x.fecha, tipo: "CONSUMO", detalle: `ADE · lote ${x.lote_id} · ${x.animales} animales` });
  ...
  const esAlimento = x => {
    const t = (x.nombre + " " + (x.unidad || "")).toUpperCase();
    return /RACION|RACIÓN|MAIZ|MAÍZ|SORGO|EXPELLER|AFRECHILLO|BALANCEADO|PELLET|SILO|FARDO|ROLLO|GRANO|NUCLEO|NÚCLEO|SAL|MINERAL/.test(t)
        || /\bKG\b|TON/.test(String(x.unidad || "").toUpperCase());
  };
  ...
  async function sincronizarPreciosImprolux(baseUrl) {
    ...
    const tx = db.transaction(() => {
      for (const it of r.items) {
        const alim = esAlimento(it);
        upProd.run(it.nombre, alim ? "ALIMENTO" : "SANIDAD", it.unidad || (alim ? "kg" : "dosis"),
                   it.costo, it.cantidad || 0);
        if (alim) res.alimentos++; else { upSan.run(it.nombre, it.costo); res.sanidad++; }
        res.detalle.push({ nombre: it.nombre, costo: it.costo, stock: it.cantidad,
                           tipo: alim ? "ALIMENTO" : "SANIDAD" });
      }
    });
    tx();
    return res;
...
function rutas(app, motores, CAMPO_DEFAULT) {
  ...
  app.get("/api/costeo/productos", (req, res) => {
    ok(res, { productos: D(req).prepare("SELECT * FROM costeo_productos ORDER BY tipo, producto").all() });
  });
